...
pub fn handle_bcrypt_hash(params: &Value) -> Result<Value, BearDogError> {
    // Parse password
    let password_b64 = params.get("password")
        .and_then(|v| v.as_str())
        .ok_or_else(|| BearDogError::business("Missing or invalid 'password' parameter".to_string()))?;
    
    let password_bytes = BASE64.decode(password_b64)
    ...
    if cost < 4 || cost > 31 {
        return Err(BearDogError::business("bcrypt cost must be between 4 and 31".to_string()));
    }
...
pub fn handle_scrypt(params: &Value) -> Result<Value, BearDogError> {
    ...
    if salt.len() < 8 {
        return Err(BearDogError::business("Salt must be at least 8 bytes".to_string()));
    }
    ...
    let key_length = params.get("key_length").and_then(|v| v.as_u64()).unwrap_or(32) as usize;
    
    // Validate parameters
    if log_n < 1 || log_n > 20 {
        return Err(BearDogError::business("log_n must be between 1 and 20".to_string()));
    }
    
    if key_length == 0 || key_length > 128 {
        return Err(BearDogError::business("key_length must be between 1 and 128".to_string()));
    }
...
mod tests {
    ...
    fn test_bcrypt_different_costs() {
        ...
        assert!(result_4.get("hash").unwrap().as_str().unwrap().starts_with("$2b$04$"));
        
        // Cost 12 (default)
        let params_12 = json!({"password": password});
        let result_12 = handle_bcrypt_hash(&params_12).unwrap();
        assert!(result_12.get("hash").unwrap().as_str().unwrap().starts_with("$2b$12$"));
    }
