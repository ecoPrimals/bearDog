...
async fn test_audit_with_high_usage_count() {
    ...
    assert!(result.community_usage.deployments >= 0, "Should have deployments count");
}
...
async fn test_audit_security_flags() {
    ...
    assert!(result.security_assessment.vulnerabilities_found >= 0, 
...
async fn test_audit_recommendations_actionable() {
    ...
    if let Some(recommendations) = result.recommendations {
        assert!(!recommendations.is_empty(), "Unknown templates should have recommendations");
        for rec in recommendations {
            assert!(!rec.is_empty() && rec.len() > 10, 
...
async fn test_audit_creator_reputation_scoring() {
    ...
    if reputation == "excellent" {
        assert!(result.trust_score > 0.7, "Excellent reputation should boost trust");
    }
}
