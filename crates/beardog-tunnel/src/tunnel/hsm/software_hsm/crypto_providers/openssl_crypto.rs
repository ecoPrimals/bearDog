...
use crate::tunnel::hsm::types::{Algorithm, KeyType};
...
impl CryptoProvider for OpenSslCryptoProvider {
    ...
    async fn sign(&self, key_material: &[u8], data: &[u8]) -> Result<Vec<u8>, BearDogError> {
        ...
        Err(BearDogError::Crypto {
            message: "OpenSSL Ed25519 signing not yet implemented".to_string(),
        })
    }
    ...
    async fn verify(
        ...
        Err(BearDogError::Crypto {
            message: "OpenSSL Ed25519 verification not yet implemented".to_string(),
        })
    }
...
mod tests {
    ...
    async fn test_key_generation() -> Result<(), BearDogError> {
        let provider = OpenSslCryptoProvider::new().await?;
        let key_material = provider.generate_key_material(&KeyType::Aes256).await?;
        assert_eq!(key_material.len(), 32);

        let ecc_key = provider.generate_key_material(&KeyType::EccP256).await?;
    ...
    async fn test_key_derivation() -> Result<(), BearDogError> {
        ...
        let different_key = provider.derive_key(root_key, different_derivation_data).await?;
