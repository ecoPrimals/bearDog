    ...
    fn safe_check_availability() -> bool {
        ...
        let supports_secure_enclave = matches!(device_type.as_str(),
            device if device.contains("iPhone") && Self::parse_iphone_version(&device) >= 5 ||
                     device.contains("iPad") && Self::parse_ipad_version({}, Secure Enclave supported: {}",
    ...
    fn parse_ipad_version(device: &str) -> u32 {
        ...
        Ok(HsmKey {
            ...
            metadata: KeyMetadata {
                ...
                key_name: Some(Some(format!("{:?}", key_type)),
                key_size: Some(256),
        ...
        let signature = BearDogCrypto::sign_ed25519({} bytes", signature.len(&[u8],
        signature: &[u8],
        ...
        let is_valid =
            BearDogCrypto::verify_ed25519_signature({}", is_valid);
        Ok(is_valid)
...
mod tests {
    use super::*;
    #[tokio::test]
    fn test_safe_secure_enclave_creation() -> Result<(), BearDogError> {
        let enclave = SafeSecureEnclave::new();
        assert!(enclave.is_ok());
        Ok(())}


    fn test_safe_key_generation() -> Result<(), BearDogError> {
        let enclave = SafeSecureEnclave::new().map_err(|e| {
