...
impl SafeSecureEnclave {
    ...
    async fn safe_check_availability() -> bool {
        ...
        if ios_version < 9.0 {
            warn!("iOS version {} < 9.0, Secure Enclave not supported", ios_version);
            return false;
        }
        ...
        info!("🍎 iOS device: {}, Secure Enclave supported: {}", device_type, supports_secure_enclave);
    ...
    fn parse_iphone_version(device: &str) -> u32 {
        if device.contains("iPhone15") { 15 }
        else if device.contains("iPhone14") { 14 }
        else if device.contains("iPhone13") { 13 }
        else if device.contains("iPhone12") { 12 }
        else if device.contains("iPhone11") { 11 }
        else if device.contains("iPhone10") { 10 }
        else if device.contains("iPhone9") { 9 }
        else if device.contains("iPhone8") { 8 }
        else if device.contains("iPhone7") { 7 }
        else if device.contains("iPhone6") { 6 }
        else if device.contains("iPhone5") { 5 }
        else { 4 }
    }

    /// Parse iPad version from device string
    fn parse_ipad_version(device: &str) -> u32 {
        if device.contains("iPad10") { 10 }
        else if device.contains("iPad9") { 9 }
        else if device.contains("iPad8") { 8 }
        else if device.contains("iPad7") { 7 }
        else if device.contains("iPad6") { 6 }
        else if device.contains("iPad5") { 5 }
        else if device.contains("iPad4") { 4 }
        else if device.contains("iPad3") { 3 }
        else { 2 }
    }

    /// Generate key using safe methods instead of unsafe Security Framework calls
    pub async fn safe_generate_key(&self, key_id: &str, key_type: &KeyType) -> BearDogResult<HsmKey> {
        info!("🔐 Safe key generation for iOS Secure Enclave: {}", key_id);

        if !self.available {
            return Err(BearDogError::NotAvailable {
        ...
        let keypair = match key_type {
            ...
            _ => {
                return Err(BearDogError::NotSupported {
                    message: format!("Key type {:?} not supported in safe iOS implementation", key_type),
                });
            }
        };
        ...
        Ok(HsmKey {
            ...
            private_key: None, // Private key stays in "secure enclave" (software simulation)
            hardware_backed: self.available,
            attestation_certificate: Some(self.generate_safe_attestation_cert(key_id)?),
        })
    }
    ...
    pub async fn safe_verify_signature(&self, key_id: &str, data: &[u8], signature: &[u8]) -> BearDogResult<bool> {
        info!("🔍 Safe signature verification for iOS Secure Enclave: {}", key_id);
...
mod tests {
    ...
    #[tokio::test]
    async fn test_safe_secure_enclave_creation() {
        let enclave = SafeSecureEnclave::new().await;
        assert!(enclave.is_ok());
    }

    #[tokio::test]
    async fn test_safe_key_generation() {
        let enclave = SafeSecureEnclave::new().await.unwrap();
        if enclave.is_available() {
            let key = enclave.safe_generate_key("test_key", &KeyType::Ed25519).await;
            assert!(key.is_ok());
        }
    }
