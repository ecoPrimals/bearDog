...
//! [`HsmKeyProvider`] implementation for the Linux Secret Service backend.

use super::{LinuxSecretServiceHsm, SecretKeyEntry};
...
impl HsmKeyProvider for LinuxSecretServiceHsm {
        ...
        async move {
            if !self.is_available() {
                return Err(BearDogError::not_yet_available(
                    "Linux Secret Service HSM is not available on this platform",
                ));
            }
            ...
            let encrypted = Self::encrypt_at_rest(&key_material)?;
            self.store_blob(&key_id, &encrypted).await?;
    ...
    let size = match algorithm {
        ...
        HsmAlgorithm::Rsa2048 | HsmAlgorithm::Rsa4096 => {
            return Err(BearDogError::internal(
                "RSA key generation not supported in secret-service backend".to_string(),
            ));
        }
    };
