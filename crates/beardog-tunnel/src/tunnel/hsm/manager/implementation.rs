...
pub struct DefaultHsmManager {
    ...
    pub hsm_providers: HashMap<String, Box<crate::tunnel::hsm::HsmProviderBackend>>,
}
...
impl DefaultHsmManager {
    ...
    pub fn register_provider(
        &mut self,
        id: String,
        provider: Box<crate::tunnel::hsm::HsmProviderBackend>,
    ) -> Result<(), BearDogError> {
    ...
    pub fn get_provider(
        &self,
        id: &str,
    ) -> Result<&crate::tunnel::hsm::HsmProviderBackend, BearDogError> {
        self.hsm_providers
            .get(id)
...
/// Unit tests for [`DefaultHsmManager`] / [`HsmProvider`].
#[cfg(test)]
mod tests {
    use super::*;
    use crate::tunnel::hsm::HsmProviderBackend;
    use crate::tunnel::hsm::hsm_provider_mocks::DefaultManagerMockProvider;
    ...
    async fn test_register_provider() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let provider = Box::new(HsmProviderBackend::DefaultManagerMock(
            DefaultManagerMockProvider {
                id: "mock-1".to_string(),
            },
        ));
    ...
    async fn test_provider_info() -> Result<(), Box<dyn std::error::Error>> {
        let provider = HsmProviderBackend::DefaultManagerMock(DefaultManagerMockProvider {
            id: "mock-1".to_string(),
        });
