...
use super::*;
use crate::{EcosystemResult, EcosystemError};
...
impl ServiceDiscoveryClient {
    ...
    pub async fn discover_by_capability(&mut self, capability: &str) -> EcosystemResult<Vec<UniversalServiceRegistration>> {
        // Check cache first
        ...
        self.cache.by_capability.insert(capability.to_string(), cache_entry);
        
        Ok(results)
    }
    
    /// Get service by ID with caching
    pub async fn get_service(&mut self, service_id: uuid::Uuid) -> EcosystemResult<Option<UniversalServiceRegistration>> {
        // Check cache first
    ...
    pub async fn register_service(&self, registration: &UniversalServiceRegistration) -> EcosystemResult<()> {
    ...
    fn count_expired_entries(&self) -> usize {
        let now = Instant::now();
        let capability_expired = self.cache.by_capability.values()
            .filter(|entry| entry.expires_at <= now)
            .count();
        let service_expired = self.cache.by_service_id.values()
            .filter(|entry| entry.expires_at <= now)
            .count();
...
pub trait DiscoveryBackend: Send + Sync + std::fmt::Debug {
    ...
    async fn discover_by_capability(&self, capability: &str) -> EcosystemResult<Vec<UniversalServiceRegistration>>;
    
    /// Get specific service by ID
    async fn get_service(&self, service_id: uuid::Uuid) -> EcosystemResult<Option<UniversalServiceRegistration>>;
    
    /// Register a service
    async fn register_service(&self, registration: &UniversalServiceRegistration) -> EcosystemResult<()>;
...
impl InMemoryDiscoveryBackend {
    ...
    pub fn new() -> Self {
        Self {
            registry: tokio::sync::RwLock::new(super::capability_registry::CapabilityRegistry::new()),
        }
    }
}
...
impl DiscoveryBackend for InMemoryDiscoveryBackend {
    ...
    async fn get_service(&self, service_id: uuid::Uuid) -> EcosystemResult<Option<UniversalServiceRegistration>> {
        ...
        Ok(all_services.into_iter().find(|s| s.service_id == service_id))
    }
