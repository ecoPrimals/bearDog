...
fn test_capability_response_with_metadata() {
    ...
    assert_eq!(response.metadata.get("execution_time"), Some(&"100ms".to_string()));
}
...
fn test_ai_integration_response_serialization() {
    ...
    assert_eq!(response.suggested_actions.len(), deserialized.suggested_actions.len());
}
...
fn test_universal_adapter_register_capability() {
    ...
    adapter.register_capability("encryption".to_string(), "http://encryption.svc".to_string());
    adapter.register_capability("signing".to_string(), "http://signing.svc".to_string());
    
    assert_eq!(adapter.get_capabilities().len(), 2);
    assert!(adapter.get_capabilities().contains(&"encryption".to_string()));
