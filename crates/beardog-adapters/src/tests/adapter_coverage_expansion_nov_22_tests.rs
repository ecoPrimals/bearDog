...
mod adapter_coverage_tests {
    ...
    async fn test_adapter_retry_logic() {
        ...
        assert_eq!(attempt, max_retries, "Should retry expected number of times");
    }
    ...
    async fn test_adapter_fallback_mechanism() {
        ...
        let capabilities = adapter.get_capabilities();
        assert!(capabilities.len() >= 2, "Should have both primary and fallback");
    }
    ...
    async fn test_adapter_load_balancing() {
        ...
        let capabilities = adapter.get_capabilities();
        assert_eq!(capabilities.len(), 3, "Should have all instances registered");
    }
}
