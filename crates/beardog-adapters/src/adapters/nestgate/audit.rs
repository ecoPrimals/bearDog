...
impl AuditManager {
    ...
    fn matches_filter(&self, event: &NestGateAuditEvent, filter: &AuditFilter) -> bool {
        ...
        if !filter.event_types.is_empty()
            && !filter.event_types.contains(&"*".to_string())
            && !filter
                .event_types
                .iter()
        ...
        if !filter.user_ids.is_empty()
            && !filter.user_ids.contains(&"*".to_string())
            && !filter
                .user_ids
                .iter()
        ...
        // Check provider ID
        if !filter.provider_ids.is_empty()
            && !filter.provider_ids.contains(&"*".to_string())
            && !filter
                .provider_ids
                .iter()
        ...
        if !filter.resources.is_empty()
            && !filter.resources.contains(&"*".to_string())
            && !filter
                .resources
                .iter()
    ...
    pub async fn health_check(&self) -> NestGateResult<HealthStatus> {
        ...
        let _stats = self.statistics.read().await;
