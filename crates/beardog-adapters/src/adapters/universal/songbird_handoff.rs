    ...
    fn create_service_registration(
        ...
        let service_capabilities = ServiceCapabilities {
            ...
            performance: PerformanceCapabilities {
                latency_ms: Some(100),
                throughput_rps: Some(beardog_types::constants::domains::system::defaults::DEFAULT_QUEUE_SIZE),
                max_concurrent_requests: Some(100),
            },
        };

        // Helper function to get host with constants fallback
        let get_host = || {
            std::env::var("BEARDOG_HOST").unwrap_or_else(|_| {
                beardog_types::constants::domains::network::addresses::LOCALHOST_IPV4.to_string()
            })
        };
        ...
        let endpoints = ServiceEndpoints {
            primary: std::env::var("SERVICE_MESH_ENDPOINT")
                .unwrap_or_else(|_| {
                    let host = get_host();
                    let port = std::env::var("BEARDOG_API_PORT").unwrap_or_else(|_| {
                        beardog_types::constants::domains::network::ports::DEFAULT_API_PORT.to_string()
                    });
            ...
            health: std::env::var("SERVICE_MESH_HEALTH_ENDPOINT")
                .unwrap_or_else(|_| {
                    let host = get_host();
                    let port = std::env::var("BEARDOG_HEALTH_PORT").unwrap_or_else(|_| {
                        beardog_types::constants::domains::network::ports::DEFAULT_HEALTH_PORT.to_string()
                    });
            ...
            metrics: std::env::var("SERVICE_MESH_METRICS_ENDPOINT")
                .unwrap_or_else(|_| {
                    let host = get_host();
                    let port = std::env::var("BEARDOG_METRICS_PORT").unwrap_or_else(|_| {
                        beardog_types::constants::domains::network::ports::DEFAULT_METRICS_PORT.to_string()
                    });
            ...
            websocket: Some(std::env::var("SERVICE_MESH_WS_ENDPOINT")
                .unwrap_or_else(|_| {
                    let host = get_host();
                    let port = std::env::var("BEARDOG_WS_PORT").unwrap_or_else(|_| {
                        beardog_types::constants::domains::network::ports::DEFAULT_API_PORT.to_string()
                    });
        ...
        let resource_requirements = ResourceSpec {
            cpu_cores: Some(1.0),
            memory_mb: Some(beardog_types::constants::domains::system::defaults::DEFAULT_CACHE_SIZE),
            ...
            let capability = super::traits::Capability {
                ...
                resource_requirements: super::traits::ResourceRequirements {
                    ...
                    memory: Some(super::traits::ResourceRequirement {
                        min: beardog_types::constants::domains::system::defaults::DEFAULT_CACHE_SIZE,
                        max: Some(1024),
                        unit: "MB".to_string(),
                    }),
                    storage: Some(super::traits::ResourceRequirement {
                        min: 1,
                        max: Some(beardog_types::constants::domains::system::defaults::DEFAULT_POOL_SIZE),
                        unit: "GB".to_string(),
                    }),
                    network: Some(super::traits::ResourceRequirement {
                        min: 100,
                        max: Some(beardog_types::constants::domains::system::defaults::DEFAULT_QUEUE_SIZE),
                        unit: "Mbps".to_string(),
                    }),
                    custom: HashMap::new(),
                },
