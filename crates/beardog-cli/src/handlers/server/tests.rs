...
use super::gateway::bind_gateway_listener;
...
#[tokio::test]
async fn bind_gateway_listener_succeeds_on_free_port() {
    let listener = bind_gateway_listener(0).await.expect("bind port 0");
    let local = listener.local_addr().expect("local_addr");
    assert_ne!(local.port(), 0, "OS should assign a real port");
}

#[tokio::test]
async fn bind_gateway_listener_fails_on_occupied_port() {
    let first = bind_gateway_listener(0).await.expect("first bind");
    let port = first.local_addr().expect("addr").port();
    let result = bind_gateway_listener(port).await;
