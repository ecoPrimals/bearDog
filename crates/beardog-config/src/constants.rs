...
pub mod hsm {
    ...
    pub const DEFAULT_KEY_CACHE_TTL: std::time::Duration = std::time::Duration::from_secs(300);
    // 5 minutes
}
...
pub mod compliance {
    ...
    pub const DEFAULT_AUDIT_RETENTION: std::time::Duration =
        std::time::Duration::from_secs(31536000); // 1 year

    /// Default compliance check interval
    pub const DEFAULT_COMPLIANCE_CHECK_INTERVAL: std::time::Duration =
...
pub mod threat_detection {
    ...
    pub const DEFAULT_THREAT_SCAN_INTERVAL: std::time::Duration =
...
pub mod workflow {
    ...
    pub const DEFAULT_WORKFLOW_TIMEOUT: std::time::Duration = std::time::Duration::from_secs(86400);
    // 24 hours
}
