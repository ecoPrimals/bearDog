...
impl BearDogConfig for UnifiedAdapterConfig {
    fn validate(&self) -> BearDogResult<()> {
        ...
        validation::validate_non_empty_collection(&self.discovery.endpoints, "discovery_endpoints")?;
    ...
    fn merge(&self, other: &Self) -> BearDogResult<Self> {
        ...
        if other.core.adapter_id != "default-adapter" {
            merged.core.adapter_id = other.core.adapter_id.clone();
        }
        ...
        if !other.discovery.endpoints.is_empty() {
            merged.discovery.endpoints = other.discovery.endpoints.clone();
        }
    ...
    fn from_env() -> BearDogResult<Self> {
        ...
        if let Ok(max_conn) = std::env::var("BEARDOG_ADAPTER_MAX_CONNECTIONS") {
            config.core.max_connections = max_conn.parse()
        ...
        if let Ok(endpoints) = std::env::var("BEARDOG_ADAPTER_DISCOVERY_ENDPOINTS") {
            config.discovery.endpoints = endpoints.split(',').map(|s| s.trim().to_string()).collect();
        }
...
pub mod migration {
    use super::{UnifiedAdapterConfig, Duration, DiscoveryConfig, OptimizationConfig};
...
mod tests {
    ...
    fn test_migration_utilities() {
        let migrated = migration::migrate_legacy_adapter_config(
