...
/// Resolve discovery HTTP endpoint from environment (empty when unconfigured).
fn resolve_discovery_endpoint_from_env() -> String {
    ...
    if let Ok(host) = std::env::var("DISCOVERY_HOST") {
...
/// Resolve service registry endpoint from environment (empty when unconfigured).
fn resolve_registry_endpoint_from_env() -> String {
    if let Ok(endpoint) = std::env::var("BEARDOG_SERVICE_REGISTRY_ENDPOINT") {
        return endpoint;
    }
    ...
    if let Ok(host) = std::env::var("REGISTRY_HOST") {
        let port =
            std::env::var("REGISTRY_PORT").unwrap_or_else(|_| DEFAULT_CONSUL_HTTP_PORT.to_string());
        return format!("http://{host}:{port}");
    }
    String::new()
}
...
impl UnifiedDiscoveryConfig {
    ...
    /// Returns true when at least one HTTP discovery endpoint is configured.
    #[must_use]
    pub fn is_configured(&self) -> bool {
        self.enabled_protocols.iter().any(|protocol| {
            matches!(
                protocol,
                DiscoveryProtocol::Http { endpoint, .. } if !endpoint.is_empty()
            )
        })
    }
}
...
impl Default for ServiceRegistryConfig {
    fn default() -> Self {
        let registry_endpoint = resolve_registry_endpoint_from_env();
        let endpoints = if registry_endpoint.is_empty() {
            Vec::new()
        } else {
            vec![registry_endpoint]
        };
...
impl BearDogConfig for UnifiedDiscoveryConfig {
    fn validate(&self) -> Result<(), BearDogError> {
        if self.enabled {
            ...
            // Registry validation (only when HTTP discovery is configured)
