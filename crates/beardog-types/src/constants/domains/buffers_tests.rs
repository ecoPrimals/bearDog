...
mod buffer_tests {
    ...
    fn test_buffer_sizes_are_powers_of_two() {
        ...
        assert!(BUFFER_SIZE_SMALL.is_power_of_two(), "BUFFER_SIZE_SMALL (1KB) must be power of 2");
        assert!(BUFFER_SIZE_MEDIUM.is_power_of_two(), "BUFFER_SIZE_MEDIUM (4KB) must be power of 2");
        assert!(BUFFER_SIZE_LARGE.is_power_of_two(), "BUFFER_SIZE_LARGE (16KB) must be power of 2");
        assert!(BUFFER_SIZE_XLARGE.is_power_of_two(), "BUFFER_SIZE_XLARGE (64KB) must be power of 2");
    }

    #[test]
    fn test_network_buffers_are_powers_of_two() {
        assert!(NETWORK_BUFFER_SIZE.is_power_of_two(), "NETWORK_BUFFER_SIZE must be power of 2");
        assert!(TCP_BUFFER_SIZE.is_power_of_two(), "TCP_BUFFER_SIZE must be power of 2");
        assert!(UDP_PACKET_SIZE.is_power_of_two(), "UDP_PACKET_SIZE must be power of 2");
    }
    ...
    fn test_default_buffer_is_medium() {
        assert_eq!(BUFFER_SIZE_DEFAULT, BUFFER_SIZE_MEDIUM, 
    ...
    fn test_file_buffer_values() {
        ...
        assert_eq!(LARGE_FILE_BUFFER_SIZE, 1_048_576, "Large file buffer should be 1MB");
    }
    ...
    fn test_bstp_buffer_size() {
        ...
        assert!(BSTP_BUFFER_SIZE.is_power_of_two(), "BSTP buffer must be power of 2");
    }

    #[test]
    fn test_discovery_buffer_size() {
        assert_eq!(DISCOVERY_BUFFER_SIZE, 8_192, "Discovery buffer should be 8KB");
    ...
    fn test_pool_sizes_are_reasonable() {
        ...
        assert!(total_prealloc < 2 * 1024 * 1024, 
    ...
    fn test_max_buffer_size_reasonable() {
        assert_eq!(MAX_BUFFER_SIZE, 10 * 1024 * 1024, "Max buffer should be 10MB");
    ...
    fn test_zero_copy_buffer_size() {
        assert_eq!(ZERO_COPY_BUFFER_SIZE, 65_536, "Zero-copy buffer should be 64KB");
        assert_eq!(ZERO_COPY_BUFFER_SIZE, NETWORK_BUFFER_SIZE,
            "Zero-copy should match network buffer for efficiency");
    }

    #[test]
    fn test_dma_buffer_alignment() {
        assert_eq!(DMA_BUFFER_SIZE, 4_096, "DMA buffer should be 4KB (page size)");
    ...
    fn test_deprecated_aliases_match() {
        assert_eq!(DEFAULT_BUFFER_SIZE_NETWORK, NETWORK_BUFFER_SIZE,
            "Deprecated alias must match current constant");
        assert_eq!(DEFAULT_BUFFER_SIZE_FILE, FILE_BUFFER_SIZE,
            "Deprecated alias must match current constant");
    }
    ...
    fn test_buffer_invariants() {
        ...
        for &buffer in &buffers {
            assert!(buffer >= MIN_BUFFER_SIZE, 
                "Buffer {} must be >= MIN_BUFFER_SIZE", buffer);
            assert!(buffer <= MAX_BUFFER_SIZE,
                "Buffer {} must be <= MAX_BUFFER_SIZE", buffer);
        }
    }
    ...
    fn test_udp_packet_size_below_mtu() {
        ...
        assert!(UDP_PACKET_SIZE > TYPICAL_MTU,
    ...
    fn test_hsm_buffer_appropriate_for_pkcs11() {
        ...
        assert_eq!(HSM_BUFFER_SIZE, 4_096,
