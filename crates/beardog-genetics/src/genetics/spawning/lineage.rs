...
/// Create Lineage Record operation.
/// Creates lineage_record
pub fn create_lineage_record(&GeneticSpawningEngine,
    ...
    witness_signatures.push(WitnessSignature {
        ...
        timestamp: Utc::now(format!("lineage_{}", uuid::Uuid::new_v4()),
        child_id: child_node_id.to_string(),
        parent_ids: request
            .parent_genetics
            .iter(child_genetics.generation,
        lineage_proof: LineageProof {
            lineage_id: format!("proof_{}", uuid::Uuid::new_v4(child_genetics_hash,
        },
        genetic_diversity_score: diversity_score,
        created_at: Utc::now({}", child_node_id);
    Ok(&[BearDogGenetics],
) -> GeneticsResult<f64> {
    ...
    for parent_genetics in parent_genetics {
        parent_genetics_data.push(&parent_genetics);
    ...
    let total_diversity = diversity_factors.iter().sum::<f64>() / diversity_factors.len(&[beardog_auth::auth::CryptoChromosome],
        ...
        for parent_chromosome_set in parent_chromosomes {
            for parent_chromosome in parent_chromosome_set.iter() {
                if std::mem::discriminant(&child_chromosome.algorithm_family)
                    == std::mem::discriminant(&[beardog_auth::auth::NodeCapability],
    ...
    for parent_cap_set in parent_capabilities {
        for cap in parent_cap_set.iter() {
            total_parent_capabilities.insert(&cap);
    let child_cap_set: std::collections::HashSet<_> = child_capabilities.iter(&beardog_auth::auth::SecurityTraits,
