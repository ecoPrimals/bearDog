...
impl SpawnValidation {
    ...
    pub async fn validate_genetic_signature(genetics: &BearDogGenetics) -> BearDogResult<()> {
            ...
            SecurityClearance::Medium => {
                ...
                if genetics.crypto_chromosomes.len() < 2 {
                    return Err(BearDogError::Authorization {
                        message: "Medium clearance requires at least 2 crypto chromosomes"
                            .to_string(),
                    });
                }
                Ok(())
        ...
        for (idx, chromosome) in genetics.crypto_chromosomes.iter().enumerate() {
            ...
            if chromosome.security_level == 0 {
                return Err(BearDogError::Validation {
                    message: format!("Chromosome {idx} has zero security level"),
                });
            }
        }
        ...
        for capability in required_capabilities {
            ...
            match capability {
                ...
                NodeCapability::ComputeProvider => {
                    ...
                    if crypto_count == 0 {
                        return Err(BearDogError::Validation {
                            message: "Compute provider capability requires crypto chromosomes"
                                .to_string(),
                        });
                    }
                    info!(
    ...
    pub fn validate_resource_limits(
        ...
        if limits.max_cpu_percent > 100 || limits.max_cpu_percent == 0 {
            return Err(BearDogError::Validation {
                message: format!("Invalid CPU limit: {}%", limits.max_cpu_percent),
            });
        }
    ...
    /// Validate genetic lineage
    pub fn validate_genetic_lineage(genetics: &BearDogGenetics) -> BearDogResult<()> {
        ...
        if let Some(ref parent_genetics) = genetics.parent_genetics {
            for (idx, parent_id) in parent_genetics.iter().enumerate() {
                if parent_id.is_empty() {
                    return Err(BearDogError::Validation {
                        message: format!("Empty parent ID at index {idx}"),
                    });
                }
