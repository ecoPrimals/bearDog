...
fn create_test_operation(op_type: OperationType, path: Option<&str>) -> KeyOperation {
    match op_type {
        OperationType::Encrypt => KeyOperation::Encrypt {
            recipients: vec![],
        },
        OperationType::Decrypt => KeyOperation::Decrypt {
            domain: None,
        },
        OperationType::Sign => KeyOperation::Sign {
            domain: None,
        },
...
fn test_use_count_exceeded_fails() {
    ...
    match result {
        Err(ConstraintViolationError::UseCountExceeded { max_uses, current_uses }) => {
...
fn test_multiple_operations_tracking() {
    ...
    assert!(ConstraintEnforcer::verify_operation(&signed1, &operation, verifying_key.as_bytes()).is_ok());
    ...
    assert!(ConstraintEnforcer::verify_operation(&signed2, &operation, verifying_key.as_bytes()).is_ok());
    ...
    assert!(ConstraintEnforcer::verify_operation(&signed3, &operation, verifying_key.as_bytes()).is_ok());
    ...
    assert!(ConstraintEnforcer::verify_operation(&signed4, &operation, verifying_key.as_bytes()).is_err());
}
...
fn test_limited_scope_domain_check() {
    ...
    assert!(matches!(signed.constraints.scope, ScopeConstraint::Limited { .. }));
}
...
fn test_forbidden_scope_domain() {
    ...
    // Verify the constraints were set correctly
    assert!(matches!(signed.constraints.scope, ScopeConstraint::Forbidden { .. }));
}
