    ...
    fn default() -> Self {
        Self {
            discovery_timeout_ms:
                beardog_types::constants::domains::network::defaults::DEFAULT_CONNECTION_TIMEOUT
                    .as_millis() as u64, // 30 seconds
            max_discovery_attempts: 10,
            min_capabilities_threshold: 1, // At least discover one other capability
            listen_interface: beardog_types::constants::domains::network::addresses::WILDCARD_IPV4
                .to_string(),
...
impl ZeroKnowledgeBootstrap {
    ...
    fn discover_ecosystem_capabilities(&mut self) -> BearDogResult<()> {
        ...
        while discovery_attempts < self.config.max_discovery_attempts {
            ...
            std::thread::sleep(std::time::Duration::from_millis(
                beardog_types::constants::domains::system::defaults::DEFAULT_QUEUE_SIZE as u64,
            ));
        }
    ...
    async fn build_capability_registry(&mut self) -> BearDogResult<()> {
        ...
        for (service_capability_type, providers) in capabilities.iter() {
            for _provider in providers {
                // Convert ServiceCapabilityType to CapabilityType for registration
                let capability_type = match service_capability_type {
                    ServiceCapabilityType::ArtificialIntelligence => beardog_types::canonical::capabilities::CapabilityType::ArtificialIntelligence,
                    ServiceCapabilityType::Compute => beardog_types::canonical::capabilities::CapabilityType::Compute,
                    ServiceCapabilityType::Storage => beardog_types::canonical::capabilities::CapabilityType::CloudStorage,
                    ServiceCapabilityType::Security => beardog_types::canonical::capabilities::CapabilityType::Authentication,
                    ServiceCapabilityType::Network => beardog_types::canonical::capabilities::CapabilityType::LoadBalancing,
                    _ => {
                        // Default mapping for other service capability types
                        debug!("Using default capability mapping for {:?}", service_capability_type);
                        beardog_types::canonical::capabilities::CapabilityType::Authentication
                    }
                };
                self.capability_registry
                    .register_capability(service_capability_type.to_string(), capability_type)
                    .await?;
            }
        }

        info!(
            "✅ Capability registry built with {} capability types",
            capabilities.len()
        );
        Ok(())
    }
...
mod tests {
    ...
    async fn test_infant_learning_pattern() {
        ...
        assert!(
            !state.discovered_capabilities.read().is_empty() || state.ecosystem_listener.is_some(),
