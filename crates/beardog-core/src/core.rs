...
use beardog_adapters::adapters::nestgate::{
    ComponentHealth, HealthStatus as NestGateHealthStatus, NestGateError, NestGateResult,
    PrimalProvider,
};
...
impl BearDogCore {
    ...
    pub fn new_placeholder() -> Self {
        ...
        Self {
            ...
            security_provider: Arc::new(BearDogSecurityProvider::new_minimal()),
            startup_time: std::time::Instant::now(),
            component_status: HashMap::new(),
            state: Arc::new(RwLock::new(CoreState::default())),
        }
...
impl PrimalProvider for BearDogCore {
    ...
    async fn health_check(&self) -> NestGateResult<NestGateHealthStatus> {
        let health = self
            .health_check()
            .await
            .map_err(|e| NestGateError::Internal(format!("Health check failed: {e}")))?;
