...
// RecoveryConfig removed - use UnifiedRecoveryConfig instead
        }
    }
...
impl RecoveryManager {}
    ...
    pub async fn generate_ephemeral_key(
        ...
        Ok(key)
    pub async fn validate_recovery_key(&self, key_id: &str) -> Result<bool, BearDogError> {
        if let Some(key) = self.active_keys.get(key_id) {
            let now = chrono::Utc::now();
            Ok(key.is_active && now < key.expires_at)
        } else {
            Ok(false)
    async fn generate_secure_key(&self) -> Result<Vec<u8>, BearDogError>> {
