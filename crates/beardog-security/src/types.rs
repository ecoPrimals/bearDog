...
pub struct SecurityProviderConfig {
    ...
    /// Enable in-memory key manager for standalone operation
    pub enable_memory_key_manager: bool,
    /// Maximum number of keys to store in memory
    pub max_memory_keys: usize,
    /// Enable vault sharing with other BearDogs
...
pub struct BearDogSecurityProvider {
    ...
    /// Failed login attempts per user with timestamps
    pub failed_attempts: Arc<RwLock<HashMap<String, Vec<DateTime<Utc>>>>>,
    ...
    pub threat_detector: Option<beardog_threat::ThreatDetectionEngine>,
    /// Optional workflow engine for approval processes
    pub workflow_engine: Option<beardog_workflows::MultiPartyWorkflowEngine>,
    /// Standalone in-memory key manager for "crypto in your pocket"
    pub memory_key_manager: Option<crate::memory_key_manager::MemoryKeyManager>,
    /// Recovery system for account unlock and key recovery
    pub recovery_manager: Option<crate::recovery::RecoveryManager>,
}
