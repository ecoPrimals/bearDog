...
mod tests {
    ...
    fn test_string_to_credential_id_invalid() {
        let err = Fido2MultiCredentialProvider::string_to_credential_id("not!!!valid-base64!!!")
            .unwrap_err();
    ...
    async fn test_prepare_replication_missing_credential() {
        ...
        let p = Fido2MultiCredentialProvider::new(d, None).await.unwrap();
        let err =
            MultiCredentialHsmProvider::prepare_credential_replication(&p, "missing", b"seed")
                .await
                .unwrap_err();
