...
impl BearDogSecurityProvider {
    ...
    pub async fn lock_account(&self, user_id: &str, reason: AccountLockReason) -> BearDogResult<()> {
        let mut locked_accounts = self.locked_accounts.write().await;
        locked_accounts.insert(user_id.to_string(), Utc::now());
    ...
    pub async fn unlock_account(&self, user_id: &str, admin_id: &str) -> BearDogResult<()> {
        let mut locked_accounts = self.locked_accounts.write().await;
        let was_locked = locked_accounts.remove(user_id).is_some();
    ...
    pub async fn get_locked_accounts(&self) -> BearDogResult<Vec<LockedAccountInfo>> {
        ...
        for (user_id, locked_at) in locked_accounts.iter() {
            let unlock_time = *locked_at + Duration::minutes(self.config.lockout_duration_minutes as i64);
    ...
    pub async fn cleanup_expired_locks(&self) -> BearDogResult<u32> {
        ...
        locked_accounts.retain(|_, locked_at| {
            now < *locked_at + unlock_duration
        });
