...
//!
//! Uses ECDSA P-256 (ES256) keys, required by Let's Encrypt and all major
//! ACME CAs. `EdDSA` is not supported.
...
pub struct AcmeAccount {
    /// ECDSA P-256 signing key bytes (32 bytes, secret scalar).
    #[serde(with = "hex_key")]
    secret_key: [u8; 32],
...
impl AcmeAccount {
    /// Generate a new ACME account with a fresh ECDSA P-256 keypair.
    #[must_use]
    ...
    pub fn load_or_create(path: &Path, contacts: Vec<String>) -> Result<Self, AcmeError> {
        if path.exists() {
            ...
            info!("generated new ACME account keypair (ES256)");
            Ok(account)
        }
    }
    ...
    /// Get the ECDSA P-256 signing key for this account.
    ///
    /// # Panics
    ///
    /// Panics if the stored bytes are not a valid P-256 scalar (should never
    /// happen for keys generated by [`Self::generate`]).
    #[must_use]
    #[expect(
        clippy::expect_used,
        reason = "key bytes are validated at construction; invalid scalar is a logic bug"
    )]
    pub fn signing_key(&self) -> SigningKey {
        SigningKey::from_bytes((&self.secret_key).into())
            .expect("stored key bytes are a valid P-256 scalar")
    }

    /// Get the JWK thumbprint of this account's public key.
    #[must_use]
    pub fn thumbprint(&self) -> String {
        jws::jwk_thumbprint(&VerifyingKey::from(&self.signing_key()))
    }
