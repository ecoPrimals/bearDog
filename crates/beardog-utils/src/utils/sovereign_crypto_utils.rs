...
pub mod legacy {
    ...
    /// Legacy secure random bytes (falls back to machine entropy)
    ///
    /// **DEPRECATED**: Use `SovereignCryptoUtils::secure_random_bytes` instead
    pub fn secure_random_bytes(size: usize) -> Vec<u8> {
        warn!("⚠️  Using DEPRECATED legacy secure_random_bytes - migrate to sovereign entropy");
    ...
    /// Legacy salt generation (falls back to machine entropy)
    ///
    /// **DEPRECATED**: Use `SovereignCryptoUtils::generate_salt` instead
    pub fn generate_salt() -> Vec<u8> {
        warn!("⚠️  Using DEPRECATED legacy generate_salt - migrate to sovereign entropy");
        secure_random_bytes(32)
    }

    /// Legacy nonce generation (falls back to machine entropy)
    ///
    /// **DEPRECATED**: Use `SovereignCryptoUtils::generate_nonce` instead
    pub fn generate_nonce(size: usize) -> Vec<u8> {
        warn!("⚠️  Using DEPRECATED legacy generate_nonce - migrate to sovereign entropy");
        secure_random_bytes(size)
    }

    /// Legacy password generation (falls back to machine entropy)
    ///
    /// **DEPRECATED**: Use `SovereignCryptoUtils::generate_password` instead
    pub fn generate_password(length: usize) -> String {
        warn!("⚠️  Using DEPRECATED legacy generate_password - migrate to sovereign entropy");
