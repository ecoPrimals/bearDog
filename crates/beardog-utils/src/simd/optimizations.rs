    ...
    pub fn get_aligned_buffer(&mut self, size: usize) -> Option<&mut AlignedBuffer> {
        ...
        if self.aligned_buffers.len() < 16 {
            let mut new_buffer = AlignedBuffer {
                data: Vec::with_capacity(size.max(beardog_types::constants::domains::system::defaults::DEFAULT_BUFFER_SIZE)), // Minimum 4KB
                capacity: size.max(beardog_types::constants::domains::system::defaults::DEFAULT_BUFFER_SIZE),
                in_use: true,
            };
...
mod tests {
    ...
    #[test]
    async fn test_simd_optimizer_creation() {
    ...
    #[test]
    async fn test_fast_buffer_management() {
        ...
        let buffer2 = optimizer.get_fast_buffer(beardog_types::constants::domains::system::defaults::DEFAULT_CACHE_SIZE);
        assert!(buffer2.capacity() >= beardog_types::constants::domains::system::defaults::DEFAULT_CACHE_SIZE);
    ...
    #[tokio::test]
    async fn test_aligned_buffer_management() {
        let mut optimizer = AdvancedSIMDOptimizer::new();
