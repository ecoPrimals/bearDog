...
// Simplified for Android - no heavy dependencies
...
fn test_software_hsm() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("   Assessment:      {}", if quality > 0.85 { "✅ Good" } else { "⚠️  Check" });
    println!();
...
fn test_titan_m() -> Result<(), Box<dyn std::error::Error>> {
    ...
    #[cfg(target_os = "android")]
    {
        ...
        if keystore.trim() == "trusty" {
            ...
            println!("✅ Generated 256 bytes in {:?}", elapsed);
            println!("   Shannon Entropy: {:.4} / 8.0", shannon);
            println!("   Quality Score:   {:.1}% (with hardware bonus)", quality * 100.0);
            ...
            println!("   ⚠️  Titan M not detected (keystore: {})", keystore.trim());
        }
    }
