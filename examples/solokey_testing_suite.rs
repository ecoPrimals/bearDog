    ...
    match discover_fido2_devices().await {
        Ok(devices) => {
            ...
            if devices.len() == 1 {
                ...
                for (idx, device) in devices.iter().enumerate() {
                    println!("   [{}] {} ({})", idx + 1, device.product, device.device_path.display());
                }
...
async fn test_device(device: &beardog_security::hsm::fido2::types::Fido2DeviceInfo) -> Result<(), Box<dyn std::error::Error>> {
    println!();
