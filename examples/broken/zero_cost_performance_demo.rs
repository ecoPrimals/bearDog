...
use beardog_workflows::workflows::performance_benchmarks::{
    ComprehensiveBenchmarkResults, WorkflowPerformanceBenchmarks,
};

use beardog_types::canonical::hsm::{KeyMetadata, KeyType};
use beardog_workflows::workflows::zero_cost_hsm::{
    ZeroCostHsmFactory, ZeroCostHsmProvider, ZeroCostSoftwareHsm,
};
use beardog_workflows::workflows::zero_cost_processors::{
    ZeroCostKeyRotationProcessor, ZeroCostPolicyChangeProcessor, ZeroCostWorkflowEngineFactory,
};
use std::time::Instant;

#[tokio::main]
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("[ROCKET] BearDog Zero-Cost Architecture Performance Demo");
    println!("==================================================");
    println!();

    demo_zero_cost_workflows()?;
    println!();
    ...
    demo_factory_optimizations()?;
    println!();

    println!("[OK] Zero-Cost Architecture Demo Complete!");
...
async fn demo_zero_cost_workflows() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("[OK] Created zero-cost processors:");
    ...
    println!(
        "   - Max execution time: {:?}",
        capabilities.max_execution_time
    );
    ...
    println!("[CHART] Factory processor stats:");
...
async fn demo_zero_cost_hsm() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("[OK] Created zero-cost HSM providers:");
    ...
    let key = production_hsm
        .generate_key(KeyType::Rsa2048, metadata)
        ?;
    let generation_time = start_time.elapsed({}", key.key_id);
    println!("   - Generation time: {}ms", generation_time.as_millis({} bytes signature", signature.len({}", is_valid);

    let capabilities = production_hsm.get_capabilities();
    let health = production_hsm.health_check()?;

    println!("📋 HSM capabilities:");
    println!("   - Max key size: {} bytes", capabilities.max_key_size);
    println!(
        "   - Supported algorithms: {}",
        capabilities.supported_algorithms.len()
    );
    ...
    println!(
        ...
        100.0 - health.free_storage_percent
    );
...
async fn demo_comprehensive_benchmarks() -> Result<(), Box<dyn std::error::Error>> {
    println!("[CHART] Demo 3: Comprehensive Performance Benchmarks");
    println!("-----------------------------------------------");

    println!("Running workflow processing benchmarks...");
    let workflow_results = WorkflowPerformanceBenchmarks::benchmark_workflow_processing(50);

    println!("Running HSM operations benchmarks...");
    let hsm_results = WorkflowPerformanceBenchmarks::benchmark_hsm_operations(25);

    println!("[TARGET] Performance Improvements:");
    println!(
        "   - Workflow processing: {:.1}% faster",
        workflow_results.improvement_percent
    );
    println!(
        "   - HSM operations: {:.1}% faster",
        hsm_results.improvement_percent
    );

    let average_improvement =
        (workflow_results.improvement_percent + hsm_results.improvement_percent) / 2.0;
...
async fn demo_factory_optimizations() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("[OK] High-throughput factory:");
    ...
    println!(
        "   - High-throughput key: {}",
        ZeroCostKeyRotationProcessor::<50, 60000>::PROCESSOR_NAME
    );
    println!(
        "   - Secure key: {}",
        ZeroCostKeyRotationProcessor::<10, 30000>::PROCESSOR_NAME
    );
    println!(
        "   - Policy processor: {}",
        ZeroCostPolicyChangeProcessor::<true>::PROCESSOR_NAME
    );
...
async fn compare_architectural_approaches() {
    println!("[SEARCH] Architectural Comparison:");
    println!("   async_trait approach:");
    println!("     [X] Runtime polymorphism with Box<dyn Future>");
    println!("     [X] Heap allocations for each async call");
    println!("     [X] Virtual dispatch overhead");

    println!("   Zero-cost approach:");
    println!("     [OK] Compile-time monomorphization");
    println!("     [OK] Direct function calls, no boxing");
    println!("     [OK] Optimal CPU cache utilization");
    println!("     [OK] Better compiler optimization opportunities");
}
