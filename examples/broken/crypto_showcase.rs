...
async fn main() -> Result<(), BearDogError> {
    ...
    println!("[TARGET] Crypto showcase completed successfully!");
    Ok(())
}

async fn demo_simd_hashing() -> Result<(), BearDogError> {
    println!("[LIGHTNING] SIMD-Accelerated Hashing:");

    let test_data =
        b"BearDog: Revolutionary decentralized security platform with genetic algorithms";
...
async fn demo_encryption_features() -> Result<(), BearDogError> {
    println!("[LOCK] Advanced Encryption Features:");
...
async fn demo_digital_signatures() -> Result<(), BearDogError> {
    println!("✍️ Digital Signature System:");
...
async fn demo_performance_metrics() -> Result<(), BearDogError> {
    ...
    let throughput =
        (test_data.len() as f64 * iterations as f64) / (1024.0 * 1024.0) / total_time.as_secs_f64();
