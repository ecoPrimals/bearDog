...
async fn demonstrate_ai_understanding() -> Result<(), BearDogError> {
    println!("🔐 Creating a crypto error that an AI can understand...");
    ...
    show_ai_understanding(&crypto_error);
...
async fn show_ai_understanding(error: &BearDogError) {
    ...
    match error.severity() {
        ...
        _ => println!("[OK] AI: LOW PRIORITY - Monitoring"),
    }

    if error.is_retryable(Error is retryable after {} seconds", retry_after);
        }
    }
    ...
    println!("   Automated actions: {}", automated_actions.len({}", manual_actions.len());

    if let Some(best_action) = automated_actions
        .iter()
        .min_by_key(|action| action.estimated_time_seconds.unwrap_or(u64::MAX))
    {
        ...
        println!(
            "   Estimated Time: {}s",
            best_action.estimated_time_seconds.unwrap_or({}", best_action.automatable);
    }

    let ai_report = error.to_ai_report();
    println!("📋 AI Generated Analysis:");
    println!(
        "{}",
    ...
    if error.is_critical() {
        ...
        simulate_ai_action(automated_actions[0]);
...
async fn simulate_ai_action(action: &RemediationAction) {
    println!("[LIGHTNING] AI Executing: {}", action.action_type);
    ...
    match action.action_type.as_str() {
        "regenerate_key" => {
            ...
            println!("   [OK] New key generated successfully");
        }
        _ => {
            println!("   [CYCLE] Executing action...");
            println!("   [OK] Action completed");
        }
    }
