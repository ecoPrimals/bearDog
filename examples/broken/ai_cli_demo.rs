...
async fn main() -> Result<(), BearDogError> {
    ...
    let status_response = get_system_status(&core);
    println!(
        "{}",
        serde_json::to_string_pretty(&status_response).map_err(|e| {
    ...
    println!("=== BATCH SECURITY OPERATIONS ===");
    ...
    let batch_response = process_batch_security(&core, batch_request);
    println!(
        "{}",
        serde_json::to_string_pretty(&batch_response).map_err(|e| {
            tracing::error!("Operation failed: {:?}", e);
            beardog_errors::BearDogError::internal(
                format_args!("Operation failed: {:?}", e).to_string(),
            )
    ...
    let spawn_response = process_genetic_spawn(&core, spawn_request);
    println!(
        "{}",
        serde_json::to_string_pretty(&spawn_response).map_err(|e| {
            tracing::error!("Operation failed: {:?}", e);
            beardog_errors::BearDogError::internal(
                format_args!("Operation failed: {:?}", e).to_string(),
            )
    ...
    println!("=== HSM OPERATIONS ===");
    let hsm_response = get_hsm_status(&core);
    println!(
        "{}",
        serde_json::to_string_pretty(&hsm_response).map_err(|e| {
            tracing::error!("Operation failed: {:?}", e);
            beardog_errors::BearDogError::internal(
                format_args!("Operation failed: {:?}", e).to_string(),
            )
    ...
    let benchmark_response = run_performance_benchmark(&core);
    println!(
        "{}",
        serde_json::to_string_pretty(&benchmark_response).map_err(|e| {
            tracing::error!("Operation failed: {:?}", e);
            beardog_errors::BearDogError::internal(
                format_args!("Operation failed: {:?}", e).to_string(),
            )
    ...
    println!("[OK] AI-First CLI Demo completed - all operations machine-readable!");
...
async fn process_batch_security(
    ...
    for (index, op) in request.operations.iter().enumerate() {
        ...
        let op_result = simulate_security_operation(op);
...
async fn simulate_security_operation(
    ...
    match op.operation.as_str() {
        ...
        _ => Err(BearDogError::configuration(format!(
            "Unknown operation: {}",
            op.operation
        ))),
    }
