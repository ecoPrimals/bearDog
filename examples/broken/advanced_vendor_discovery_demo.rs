...
impl UniversalVendorAdapter {
    #[must_use]
    pub const fn new(config: UniversalAdapterConfig) -> Self {
        Self {
            config,
    ...
    pub async fn discover_vendors(&mut self) -> Result<Vec<VendorInfo, BearDogError>> {
        ...
        if self.config.scan_environment {
            vendors.extend(self.scan_environment_vendors()?);
        }

        if self.config.scan_hardware {
            vendors.extend(self.scan_hardware_vendors()?);
        }

        if self.config.scan_cloud {
            vendors.extend(self.scan_cloud_vendors()?);
        }
    ...
    async fn scan_environment_vendors(&self) -> Result<Vec<VendorInfo, BearDogError>> {
        info!("[SEARCH] Scanning environment variables for vendor configurations...");
    ...
    async fn scan_cloud_vendors(&self) -> Result<Vec<VendorInfo, BearDogError>> {
        ...
        if self.check_azure_availability() {
            vendors.push(VendorInfo {
    ...
    #[must_use]
    pub fn get_vendors_by_capability(&self, capability: CapabilityOperation) -> Vec<&VendorInfo> {
    ...
    #[must_use]
    pub fn get_vendors_by_crypto_type(&self, crypto_type: CryptoOperationType) -> Vec<&VendorInfo> {
        self.discovered_vendors
            .iter()
...
async fn main() -> Result<(), BearDogError> {
    ...
    info!("[ROCKET] BearDog Advanced Vendor Discovery Demo");
    info!("==========================================");
    ...
    demonstrate_comprehensive_discovery(&mut adapter)?;

    demonstrate_capability_filtering(&adapter)?;

    demonstrate_crypto_type_filtering(&adapter)?;

    demonstrate_vendor_selection(&adapter)?;

    info!("[OK] Advanced Vendor Discovery Demo completed successfully!");
    Ok(())
}
...
async fn demonstrate_comprehensive_discovery(
    ...
    info!("[CHART] Phase 1: Comprehensive Vendor Discovery");
    info!("------------------------------------------");

    let vendors = adapter.discover_vendors()?;

    info!("[SEARCH] Discovery Summary:");
...
async fn demonstrate_capability_filtering(
    adapter: &UniversalVendorAdapter,
) -> Result<(), BearDogError> {
    info!("[TARGET] Phase 2: Capability-Based Filtering");
    info!("--------------------------------------");
...
async fn demonstrate_crypto_type_filtering(
    adapter: &UniversalVendorAdapter,
) -> Result<(), BearDogError> {
    info!("🔐 Phase 3: Crypto-Type Filtering");
    info!("----------------------------------");
...
async fn demonstrate_vendor_selection(
    adapter: &UniversalVendorAdapter,
) -> Result<(), BearDogError> {
    info!("[LIGHTNING] Phase 4: Intelligent Vendor Selection");
    info!("---------------------------------------");
    ...
    for (scenario, capability) in scenarios {
        ...
        info!(
            "   [CHART] Reasoning: {}",
            get_selection_reasoning(selected)
        );
    }
