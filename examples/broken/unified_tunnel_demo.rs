...
use beardog_tunnel::tunnel::{
    config::BStpConfig, create_gaming_tunnel_manager, create_human_entropy_hsm_manager, HsmTier,
    HumanEntropyMethod, TierElevationCriteria, UnifiedHsmManager, UnifiedHumanEntropyClassifier,
...
async fn main() -> Result<(), BearDogError> {
    ...
    info!("[ROCKET] BearDog Unified Tunnel Architecture Demo");
    info!("============================================");

    demonstrate_unified_hsm_management()?;
    demonstrate_human_entropy_system()?;
    demonstrate_tunnel_integration()?;

    info!("[OK] Unified tunnel architecture demonstration complete!");
    Ok(())
}
...
async fn demonstrate_unified_hsm_management() -> Result<(), BearDogError> {
    ...
    let hsm_manager = create_human_entropy_hsm_manager()?;

    info!("[OK] Created unified HSM manager with human entropy optimization");

    let stats = hsm_manager.get_statistics()?;
    info!("[CHART] HSM Statistics:");
    ...
    for tier in [
        ...
        info!("   - {:?} tier: {} providers", tier, providers.len(2,
    ...
    info!("[OK] Created unified human entropy classifier with custom criteria");
    ...
    info!("[TARGET] Human entropy enables:");
    ...
    info!("[OK] Unified human entropy system demonstration complete ");
    Ok(())
}

async fn demonstrate_tunnel_integration() -> Result<(), BearDogError> {
    info!("[SHIELD] Demonstrating Unified Tunnel Integration");
    info!("-------------------------------------------");

    let tunnel_manager = create_gaming_tunnel_manager()?;
    info!("[OK] Created unified tunnel manager with gaming optimization");
    ...
    let metadata = KeyMetadata {
        created_at: Utc::now(),
        algorithm: "Ed25519".to_string(256,
        usage: vec!["sign".to_string(None,
    };
    ...
    info!("[TROPHY] Unified architecture benefits:");
    ...
    info!("[OK] Unified tunnel integration demonstration complete ");
    Ok(())
}

async fn demonstrate_architecture_migration() -> Result<(), BearDogError> {
    info!("[CYCLE] Architecture Migration Benefits");
    info!("=================================");

    info!("[X] Before: Fragmented Architecture");
    ...
    info!("[OK] After: Unified Architecture");
...
mod tests {
    ...
    async fn test_unified_architecture_demo() {
        let result = tokio::spawn(async {
            let _hsm_manager = create_human_entropy_hsm_manager();

            let _tunnel_manager = create_gaming_tunnel_manager();
        ...
        for method in methods {
            match method {
                ...
                _ => {
                    ...
                    return Err(BearDogError::validation(3,
