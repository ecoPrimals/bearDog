...
async fn main() -> Result<(), BearDogError> {
    println!("[DNA] BearDog Genetic Spawning with HSM Integration Demo");
    println!("=====================================================");

    let hsm_manager = setup_hsm_manager()?;

    let genetics_store = Arc::new(InMemoryGeneticsStore::new(0.08,
    ...
    println!("📱 Demo 1: Genesis Node Creation with HSM");
    println!("==========================================");

    let genesis_node_id = "beardog-genesis-001";
    let genesis_genetics = genetics_api.create_genesis_node({}", genesis_node_id);
    println!("   Generation: {}", genesis_genetics.generation);
    println!("   Capabilities: {}", genesis_genetics.capabilities.len({}",
        genesis_genetics.crypto_chromosomes.len({:.3}", genesis_genetics.fitness_score);

    println!("[LOCK] Demo 2: Security Response Spawning");
    println!("=====================================");

    let security_spawn_request = SpawnRequest {
        request_id: Uuid::new_v4().to_string(),
        requesting_parent: genesis_node_id.to_string(vec![],
        ...
        resource_requirements: ResourceLimits {
            ...
            allowed_jurisdictions: vec!["US".to_string(vec![],
        },
        workflow_type: BearDogWorkflowType::AutomatedConsensus {
            participating_nodes: vec![genesis_node_id],
    ...
    println!(
        ...
        security_result.decision_participants.len({}", child_genetics.generation);
        println!(
            "   Enhanced capabilities: {}",
            child_genetics.capabilities.len({:?}", child_genetics.security_clearance);
        println!(
            "   Paranoia level: {}",
    ...
    println!("[LIGHTNING] Demo 3: Specialized Task Spawning");
    println!("===================================");
    ...
    println!("👥 Demo 4: Multi-Parent Genetic Recombination");
    println!("===============================================");

    let parent_a_id = "beardog-parent-a";
    let parent_a_genetics = genetics_api.create_genesis_node(parent_a_id)?;

    let parent_b_id = "beardog-parent-b";
    let parent_b_genetics = genetics_api.create_genesis_node(parent_b_id)?;

    let multi_parent_request = SpawnRequest {
        request_id: Uuid::new_v4().to_string(),
        requesting_parent: parent_a_id.to_string(vec![parent_b_id],
        purpose: SpawnPurpose::EcosystemIntegration(ResourceLimits {
        ...
        workflow_type: BearDogWorkflowType::AutomatedConsensus {
            participating_nodes: vec![parent_a_id.to_string(0.75,
    ...
    if let Some({}",
        ...
        println!(
            ...
            child_genetics
                .capabilities
                .iter()
                .any(|c| matches!(c, beardog::auth::NodeCapability::ComputeServiceCompute))
        );
    }

    println!("🔐 Demo 5: HSM-Backed Cryptographic Operations ");
    println!("==============================================");
    ...
    let random_bytes = hsm_manager
        ...
        ?;
    println!(
        "[OK] Generated {} random bytes for genetic mutations",
        random_bytes.len()
    );
    ...
    let lineage_signature = hsm_manager
        ...
        ?;
    println!(
        "[OK] Generated lineage proof signature: {} bytes",
        lineage_signature.len()
    );

    println!("[CHART] Demo 6: Genetic Diversity Analysis");
    println!("=====================================");
    ...
    println!("🏥 Demo 7: HSM Health Status");
    println!("============================");

    let hsm_health = hsm_manager.get_health_status()?;
    ...
    println!("[PARTY] Genetic Spawning Demo Complete!");
    println!("===================================");
    println!("Successfully demonstrated:");
    println!("  [OK] HSM-backed genetic operations");
    println!("  [OK] Multi-parent genetic recombination");
    println!("  [OK] Cryptographic lineage proofs");
    println!("  [OK] Secure mutation operations");
    println!("  [OK] Directed evolution for specialized tasks");
    println!("  [OK] Automated consensus workflows");
    println!("  [OK] HSM health monitoring");
...
async fn analyze_genetic_diversity(&GeneticsAPI,
    node_ids: &[&str],
) -> Result<GeneticDiversityMetrics, BearDogError> {
    let mut genetics_samples = Vec::new();

    for node_id in node_ids {
        let genetics = genetics_api.get_node_genetics(node_id)?;
