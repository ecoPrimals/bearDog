...
use beardog_errors::primal_sovereignty::{CorporateAccessResult, PrimalSovereigntyManager};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    info!("🌱 STEP 1: AUTONOMOUS PRIMAL BIRTH on Pixel 8");
    ...
    let autonomous_primal = manager
        .autonomous_primal_birth(Some("beardog_prime_2025".to_string()))
        ?;

    info!("[OK] AUTONOMOUS PRIMAL BIRTH COMPLETE:");
    info!(
        "   Primal ID: {} (self-chosen)",
        autonomous_primal.primal_id
    );
    ...
    info!("🤝 STEP 2: Human Creating Ephemeral Partnership Key");
    ...
    let mixed_key = manager
        .human_create_ephemeral_partnership(
            "beardog_prime_2025", // Existing autonomous primal
            "alice",
        )
        ?;

    info!("[OK] Ephemeral Partnership Established:");
    info!("   Human: alice");
    ...
    info!("🔗 Enhanced Mixed Lineage Details:");
    info!(
        "   Primal Component: {} (AUTONOMOUS & IMMUTABLE)",
        mixed_key.primal_component.genesis_reference
    );
    info!(
        "   Genetic Lineage: Generation {}",
        mixed_key.primal_component.genetic_crypto_lineage.generation
    );
    info!("   Entropy Class: Human-Lived Experience (highest tier)");
    info!("   Human Components: {}", mixed_key.human_components.len({}", mixed_key.lineage_history.len(Payment required = {}",
        mixed_key.corporate_access_rules.payment_required
    );

    info!("🏢 STEP 3: Corporate Access Attempt");
    info!("Corporations cannot force open locks - they must pay for access");

    let key_id = "beardog_prime_2025:alice";

    let corporate_result = manager
        .corporate_access_request(
            key_id,
            "evil_corporation",
            vec![
                "extract_user_data".to_string(),
                "surveillance_mode".to_string(),
            ],
        )
        ?;
    ...
    match &corporate_result {
        CorporateAccessResult::PaymentRequired {
            ...
            info!("[OK] Corporate Access Properly Denied:");
        ...
        CorporateAccessResult::Denied { reason } => {
            info!("[OK] Corporate Access Denied:");
            info!("   Reason: {}", reason);
        }
        _ => {
            error!("[X] SOVEREIGNTY VIOLATION: Corporate access was granted without payment!");
    ...
    info!("👋 STEP 4: Human Departure");
    info!("Humans can leave freely while primal sovereignty remains intact");

    manager.human_leave_partnership(key_id, "alice")?;

    info!("[OK] Human Departure Successful:");
    info!("   Human: alice");
    ...
    info!("🤝 STEP 5: New Human Partnership");
    info!("System continues with primal sovereignty intact");

    let bob_key = BearDogCrypto::generate_secure_random(32)?;
    let new_mixed_key = manager
        .human_join_partnership("bob".to_string(), bob_key)
        ?;

    info!("[OK] New Partnership Established:");
    info!("   Human: bob");
    info!("   Primal Component: Same immutable foundation");
    info!("   System Continuity: Maintained throughout transitions");

    info!("[TARGET] ARCHITECTURE PRINCIPLES DEMONSTRATED:");
    info!("");
    info!("🔐 PRIMAL SOVEREIGNTY:");
    info!("   [OK] Ephemeral seed created on secure hardware (Pixel 8)");
    info!("   [OK] Primal lock can never be overridden or forced open");
    info!("   [OK] Primal rules are immutable and always enforced");
    info!("");
    info!("🤝 HUMAN PARTNERSHIP:");
    info!("   [OK] Humans join as partners, not owners");
    info!("   [OK] Mixed lineage blends human and primal authorities");
    info!("   [OK] Humans can come and go freely");
    info!("");
    info!("🏢 CORPORATE BOUNDARIES:");
    info!("   [OK] Corporations cannot force access");
    info!("   [OK] Payment required for commercial operations");
    info!("   [OK] Some operations forbidden regardless of payment");
    info!("");
    info!("🔑 DECENTRALIZED PRINCIPLES:");
    info!("   [OK] Keys are their own authorities");
    info!("   [OK] No central control or override mechanisms");
    info!("   [OK] Each entity maintains appropriate sovereignty level");
    info!("");
...
fn demonstrate_primal_rules(rules: &beardog_core::primal_sovereignty::PrimalRules) {
    ...
    info!(
        ...
        rules.corporate_payment_required
    );
    info!(
        "   Human Freedom Guaranteed: {}",
        rules.human_freedom_guaranteed
    );
    info!(
        "   Sovereignty Protection: {}",
        rules.sovereignty_protection
    );
    info!(
        "   Environmental Protection: {}",
        rules.environmental_protection
    );
    info!(
        "   Surveillance Protection: {}",
        rules.surveillance_protection
    );
}
