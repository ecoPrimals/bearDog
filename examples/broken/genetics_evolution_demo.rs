...
use beardog_genetics::ecosystem_evolution::{
    EcosystemGeneticEngine, EcosystemMembership, TrustEvolution, CoordinationModel,
    BinaryAccessPattern, BinaryTrust, HierarchicalPattern, migrate_from_binary_patterns,
};
use chrono::Utc;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mock_context = beardog_genetics::ecosystem_evolution::EcosystemContext {
        current_health: beardog_genetics::ecosystem_evolution::HealthStatus::Healthy,
    ...
    let test_patterns = vec![
        ("Whitelist Pattern", BinaryAccessPattern::Whitelist {
            allowed_entities: vec![
                "alice@security-team.com".to_string(),
                "bob@dev-team.com".to_string(),
            ],
        }),
        ("Simple Allow", BinaryAccessPattern::Simple { is_allowed: true }),
        ("Simple Deny", BinaryAccessPattern::Simple { is_allowed: false }),
    ];

    for (pattern_name, pattern) in test_patterns {
        println!("🧪 Testing: {}", pattern_name);
        
        let evolved_membership = engine.evolve_access_pattern(
            pattern,
            mock_context.clone(),
        )?;
        ...
        match &evolved_membership {
            EcosystemMembership::ActiveContributor { trust_level, .. } => {
                println!("   ✅ Evolved to: ActiveContributor (trust: {:.2})", trust_level);
            },
            EcosystemMembership::LearningParticipant { trust_level, .. } => {
                println!("   🌱 Evolved to: LearningParticipant (trust: {:.2})", trust_level);
            },
            EcosystemMembership::CautiousInteraction { trust_level, .. } => {
                println!("   ⚠️  Evolved to: CautiousInteraction (trust: {:.2})", trust_level);
            },
            EcosystemMembership::EcosystemProtection { trust_level, .. } => {
                println!("   🛡️  Evolved to: EcosystemProtection (trust: {:.2})", trust_level);
            },
    ...
    for (scenario_name, trust_state) in trust_scenarios {
        ...
        let evolved_trust = engine.evolve_trust_pattern(
            trust_state,
            mock_history.clone(),
            mock_context.clone(),
        )?;
        ...
        match &evolved_trust {
            ...
            TrustEvolution::Building { progress_rate, .. } => {
                println!("   🔨 Evolved to: Building Trust (progress: {:.2})", progress_rate);
            },
            TrustEvolution::Questioning { .. } => {
                println!("   ❓ Evolved to: Questioning Trust");
            },
            TrustEvolution::Healing { recovery_progress, .. } => {
                println!("   🩹 Evolved to: Healing Trust (recovery: {:.1}%)", recovery_progress * 100.0);
            },
    ...
    let coordination_scenarios = vec![
        ("Master/Slave", HierarchicalPattern::MasterSlave {
            master: "control_node".to_string(),
            slaves: vec!["worker_1".to_string(), "worker_2".to_string()],
        }),
        ("Primary/Secondary", HierarchicalPattern::PrimarySecondary {
    ...
    println!("🧠 Detected Emergent Behaviors: {} total", emergent_behaviors.len());
    for (i, behavior) in emergent_behaviors.iter().enumerate() {
        ...
        println!("      💪 Strength: {:.1}%", behavior.emergence_strength * 100.0);
    }
    println!();
    ...
    println!("   💙 Trust Health: {:.1}%", health_report.trust_health * 100.0);
    println!("   🤝 Relationship Health: {:.1}%", health_report.relationship_health * 100.0);
    println!("   ⚙️  Coordination Health: {:.1}%", health_report.coordination_health * 100.0);
    println!();
    ...
    let migrated_memberships = migrate_from_binary_patterns(
        legacy_whitelist.clone(),
        legacy_blacklist.clone(),
    )?;

    println!("🌱 Migration Results: {} total memberships created", migrated_memberships.len());
    println!();
