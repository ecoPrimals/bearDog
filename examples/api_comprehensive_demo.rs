...
async fn demo_api_basics(client: &reqwest::Client, base_url: &str) -> Result<(), Box<dyn std::error::Error>> {
...
async fn demo_security_api(client: &reqwest::Client, base_url: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    info!("Threat Analysis Result: {}", analysis["data"]["threat_level"]);
    ...
    info!("Security Stats - Threats detected: {}", stats["data"]["threats_detected_24h"]);
    
    Ok(())
}

/// Demo Genetics API with node spawning and genetic analysis
async fn demo_genetics_api(client: &reqwest::Client, base_url: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let status: Value = client
        .get(&format!("{}/api/v1/genetics/spawn/{}", base_url, request_id))
        .send()
        .await?
        .json()
        .await?;
    ...
    let genetics_analysis: Value = client
        .get(&format!("{}/api/v1/genetics/analyze/{}", base_url, genesis_node_id))
        .send()
        .await?
        .json()
        .await?;
    
    info!("Genetic Diversity Score: {}", genetics_analysis["data"]["diversity_scores"]["overall_diversity"]);
...
async fn demo_monitoring_api(client: &reqwest::Client, base_url: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    info!("System Uptime: {} seconds", health["data"]["uptime_seconds"]);
    ...
    info!("CPU Usage: {}%", metrics["data"]["performance"]["request_latency_p95_ms"]);
    info!("Memory Usage: {}%", metrics["data"]["resources"]["memory_usage_percent"]);
...
async fn demo_realtime_monitoring(client: &reqwest::Client, base_url: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    for i in 1..=3 {
        ...
        info!("Real-time Sample {}: CPU={}%, Requests/sec={}, Response time={}ms", 
            i,
...
async fn demo_performance_features(client: &reqwest::Client, base_url: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    for i in 1..=3 {
        ...
        let processing_time = response["metadata"]["processing_time_ms"].as_u64().unwrap_or(0);
        
        info!("Request {}: Total={}ms, Processing={}ms, Cached={}", 
            i, duration.as_millis(), processing_time, cached);
...
async fn demo_error_handling(client: &reqwest::Client, base_url: &str) -> Result<(), Box<dyn std::error::Error>> {
