...
impl SoftwareHealthMonitor {
    ...
    pub async fn update_performance_metrics(&self, metrics: PerformanceMetrics) -> BearDogResult<()> {
        let mut perf_metrics = self.metrics.write().await;
        *perf_metrics = metrics;
        
        debug!("Performance metrics updated: {:.2} ops/sec", perf_metrics.operations_per_second);
        Ok(())
    }
    ...
    pub async fn record_operation(
        ...
        if !success {
            ...
            metrics.error_rate = metrics.error_rate * 0.99; // Decay error rate for successes
        }
    ...
    /// Set health status
    pub async fn set_health_status(&self, healthy: bool, error_message: Option<String>) -> BearDogResult<()> {
        let mut health_status = self.health_status.write().await;
