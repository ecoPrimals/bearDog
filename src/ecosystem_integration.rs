...
impl BearDogEcosystemProvider {
    ...
    async fn handle_auth_request(&self, request: EcosystemRequest) -> Result<EcosystemResponse, EcosystemError> {
        ...
        let auth_result = self.core.security_provider()
    ...
    async fn handle_encrypt_request(&self, request: EcosystemRequest) -> Result<EcosystemResponse, EcosystemError> {
        // Extract data to encrypt
        let data_to_encrypt = request.payload.get("data")
            .and_then(|v| v.as_str())
        ...
        let encrypted_data = self.core.encryption_engine()
            .encrypt(data_to_encrypt.as_bytes(), Some(crate::encryption::EncryptionAlgorithm::Aes256Gcm))
            .await
        ...
        Ok(EcosystemResponse {
            ...
            payload: serde_json::json!({
                "encrypted_data": base64::encode(&encrypted_data.ciphertext),
                "nonce": base64::encode(&encrypted_data.nonce),
    ...
    async fn handle_decrypt_request(&self, request: EcosystemRequest) -> Result<EcosystemResponse, EcosystemError> {
        // Extract encrypted data
        let encrypted_data_b64 = request.payload.get("encrypted_data")
            .and_then(|v| v.as_str())
            .ok_or_else(|| EcosystemError::InvalidRequest("Missing encrypted_data".to_string()))?;

        let nonce_b64 = request.payload.get("nonce")
            .and_then(|v| v.as_str())
            .ok_or_else(|| EcosystemError::InvalidRequest("Missing nonce".to_string()))?;

        // Decode base64
        let ciphertext = base64::decode(encrypted_data_b64)
            .map_err(|e| EcosystemError::InvalidRequest(format!("Invalid base64 encrypted_data: {}", e)))?;
        ...
        let decrypted_data = self.core.encryption_engine()
            .decrypt(&encrypted_data)
            .await
            .map_err(|e| EcosystemError::DecryptionFailed(e.to_string()))?;

        let decrypted_string = String::from_utf8(decrypted_data)
            .map_err(|e| EcosystemError::DecryptionFailed(format!("Invalid UTF-8 in decrypted data: {}", e)))?;
    ...
    async fn handle_compliance_request(&self, request: EcosystemRequest) -> Result<EcosystemResponse, EcosystemError> {
        // Extract compliance standard
        let standard = request.payload.get("standard")
            .and_then(|v| v.as_str())
            .unwrap_or("GDPR"); // Default to GDPR

        let resource_id = request.payload.get("resource_id")
            .and_then(|v| v.as_str())
    ...
    async fn handle_threat_scan_request(&self, request: EcosystemRequest) -> Result<EcosystemResponse, EcosystemError> {
        // Extract scan target
        let target = request.payload.get("target")
            .and_then(|v| v.as_str())
            .ok_or_else(|| EcosystemError::InvalidRequest("Missing scan target".to_string()))?;

        let scan_type = request.payload.get("scan_type")
            .and_then(|v| v.as_str())
...
impl EcosystemIntegration for BearDogEcosystemProvider {
    async fn register_with_songbird(&self) -> Result<String, EcosystemError> {
        ...
        let capability_manager = Arc::new(
            crate::adapters::universal::capability_manager::CapabilityManager::placeholder()
                .await
                .map_err(|e| EcosystemError::RegistrationFailed(e.to_string()))?
        );
        let handoff_manager = crate::adapters::universal::songbird_handoff::create_beardog_handoff_manager(
            self.core.clone(),
            capability_manager,
            None, // Use default config
        ).await.map_err(|e| EcosystemError::RegistrationFailed(e.to_string()))?;

        // Register with Songbird
        let registration_result = handoff_manager.register_with_songbird().await
            .map_err(|e| EcosystemError::RegistrationFailed(e.to_string()))?;

        if registration_result.success {
            info!("✅ Successfully registered with Songbird: {}", registration_result.service_id);
            Ok(registration_result.service_id)
        } else {
            let error_msg = registration_result.error_message
    ...
    async fn report_health(&self, health: HealthStatus) -> Result<(), EcosystemError> {
        ...
        // TODO: Implement actual health reporting to Songbird
    ...
    async fn update_capabilities(
        ...
        debug!("Updated capabilities: {:?}", capabilities.core);
