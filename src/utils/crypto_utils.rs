...
pub fn hex_to_bytes(hex: &str) -> BearDogResult<Vec<u8>> {
    if hex.len() % 2 != 0 {
        return Err(BearDogError::validation("hex", "Hex string must have even length"));
    }
    
    let mut bytes = Vec::with_capacity(hex.len() / 2);
    for chunk in hex.as_bytes().chunks(2) {
        let hex_byte = std::str::from_utf8(chunk)
            .map_err(|_| BearDogError::validation("hex", "Invalid hex character"))?;
...
mod tests {
    ...
    fn test_empty_data_handling() {
        ...
        let hmac = hmac_sha256(b"key", empty_data);
    ...
    fn test_large_data_handling() {
        ...
        let hmac = hmac_sha256(b"key", &large_data);
