...
use crate::auth::{
    AlgorithmFamily, BearDogGenetics, CryptoChromosome, EncryptionFamily, NodeCapability,
    ResourceLimits, SecurityTraits, SpawnPurpose, SpawnRequest,
};
...
impl NodeRegistry for MockNodeRegistry {
    ...
    fn register_node(&mut self, node_info: NodeInfo) -> BearDogResult<()> {
        self.trust_levels
            .insert(node_info.id.clone(), node_info.trust_level);
...
async fn test_authorization_with_insufficient_trust() -> BearDogResult<()> {
    ...
    let engine = CrossNodeAuthEngine::new(Box::new(node_registry), Box::new(MockProofVerifier));
...
async fn test_permission_security_levels() {
    assert!(
        ResourcePermission::Admin.security_level() > ResourcePermission::Delete.security_level()
    );
    assert!(
        ResourcePermission::Delete.security_level() > ResourcePermission::Write.security_level()
    );
...
async fn test_beardog_spawning() {
    ...
    assert!(spawned
        .genetics
        .capabilities
        .contains(&NodeCapability::StorageProvider));
}
...
async fn test_spawn_limits() {
    ...
    let first_spawn = engine
        .spawn_beardog(spawn_request.clone(), "test_node_1")
        .await;
...
async fn test_ecosystem_capabilities() {
    ...
    let genetics = BearDogGenetics {
        ...
        specializations: vec![NodeSpecialization::NetworkOptimized],
    };
    ...
    engine
        .spawned_beardogs
        .insert("ecosystem_spawn".to_string(), spawned);
...
async fn test_network_effects_evaluation() {
    ...
    let score = engine
        .evaluate_network_effects(&single_component)
        .await
        .unwrap();
    assert!(score > 0.0);
    assert!(score <= 1.0);
    ...
    let multi_score = engine
        .evaluate_network_effects(&multi_component)
        .await
        .unwrap();
...
async fn test_consensus_mechanism() {
    ...
    let result = engine
        .perform_consensus("test_proposal", &participating_nodes, 0.5)
        .await;
...
async fn test_authorization_cleanup() {
    ...
    engine
        ...
        .insert("expired_auth".to_string(), expired_auth);
...
async fn test_authorization_metrics() {
    ...
    engine
        .active_authorizations
        .insert("test_auth".to_string(), auth);
