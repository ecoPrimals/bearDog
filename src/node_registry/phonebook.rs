...
impl PhonebookService {
    ...
    pub async fn new(config: PhonebookConfig) -> BearDogResult<Self> {
        if !config.enable_phonebook_service {
            return Err(BearDogError::config("Phonebook service is disabled"));
        }
        
        info!("📞 Initializing Phonebook Service on {}:{}", config.bind_address, config.port);
    ...
    pub async fn register_node(&self, node_info: NodeInfo, region: String) -> BearDogResult<String> {
        info!("📝 Registering node '{}' from region '{}'", node_info.node_id, region);
        ...
        // Store the entry
        {
            ...
            nodes.insert(node_info.node_id.clone(), entry);
        }
        ...
        let registration_id = format!("reg_{}", uuid::Uuid::new_v4().to_string());
        
        info!("✅ Node '{}' registered with ID: {}", node_info.node_id, registration_id);
        Ok(registration_id)
    }
    ...
    pub async fn discover_nodes(&self, criteria: &NodeDiscoveryCriteria) -> BearDogResult<Vec<NodeInfo>> {
        ...
        info!("🔍 Found {} nodes matching discovery criteria", matching_nodes.len());
        Ok(matching_nodes)
    }
    ...
    pub async fn discover_services(&self, criteria: &ServiceDiscoveryCriteria) -> BearDogResult<Vec<ServiceAdvertisement>> {
    ...
    pub async fn cleanup_expired_entries(&self) -> BearDogResult<()> {
        ...
        // Find expired services
        {
            ...
            for (service_id, ad) in services.iter() {
                ...
                if let Ok(elapsed) = now.duration_since(
        ...
        // Update status
        {
            ...
            status.active_advertisements = status.active_advertisements.saturating_sub(expired_services.len());
            status.last_cleanup = chrono::Utc::now();
        }
        
        if !expired_nodes.is_empty() || !expired_services.is_empty() {
            info!("🧹 Cleaned up {} expired nodes and {} expired services", 
                  expired_nodes.len(), expired_services.len());
        }
    ...
    fn entry_matches_criteria(&self, entry: &PhonebookEntry, criteria: &NodeDiscoveryCriteria) -> bool {
        // Check region
    ...
    fn service_matches_criteria(&self, ad: &ServiceAdvertisement, criteria: &ServiceDiscoveryCriteria) -> bool {
        // Check service type
