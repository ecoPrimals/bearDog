...
impl BearDogNodeRegistry {
    ...
    pub async fn new(config: RegistryConfig) -> BearDogResult<Self> {
        ...
        let federation_manager = if config.federation.enable_federation {
        ...
        let phonebook_service = if config.phonebook.enable_phonebook_service {
    ...
    pub async fn add_node_with_id(
        ...
        // Update statistics
        {
            ...
            stats.total_nodes += 1;
            *stats.nodes_by_trust_level.entry(node_info.trust_level).or_insert(0) += 1;
        }
        
        // Register with phonebook service if available
        if let Some(ref phonebook) = self.phonebook_service {
            if let Err(e) = phonebook.register_node(node_info.clone(), "local".to_string()).await {
        ...
        if let Some(ref federation) = self.federation_manager {
            let advertisement = ServiceAdvertisement {
                ...
                service_name: node_info.display_name.clone(),
                service_type: node_info.node_type.clone(),
                version: "1.0.0".to_string(),
                endpoints: vec![node_info.endpoint.clone()],
        ...
        if let Some(ref federation) = self.federation_manager {
            ...
            if let Ok(federated_nodes) = federation.find_federated_nodes(&criteria).await {
                for federated_node in federated_nodes {
                    if federated_node.node_info.node_id == node_id {
    ...
    pub async fn set_trust_level(
        ...
        // Update local node
        {
            ...
            if let Some(node) = nodes.get_mut(node_id) {
                ...
                {
                    let mut stats = self.statistics.write().await;
                    *stats.nodes_by_trust_level.entry(old_trust).or_insert(0) = 
                        stats.nodes_by_trust_level.get(&old_trust).unwrap_or(&0).saturating_sub(1);
        ...
        self.trust_manager.set_trust_level(node_id, trust_level).await?;
        
        info!("🤝 Set trust level for node {} to {:?}", node_id, trust_level);
        Ok(())
    }
    ...
    pub async fn remove_node(&self, node_id: &str) -> BearDogResult<()> {
        ...
        if let Some(node) = removed_node {
            // Update statistics
            {
                ...
                *stats.nodes_by_trust_level.entry(node.trust_level).or_insert(0) = 
                    stats.nodes_by_trust_level.get(&node.trust_level).unwrap_or(&0).saturating_sub(1);
            }
    ...
    pub async fn get_federation_status(&self) -> Option<super::federation::FederationManagerStatus> {
        if let Some(ref federation) = self.federation_manager {
    ...
    pub async fn health_check(&self) -> BearDogResult<RegistryHealthStatus> {
        let mut health = RegistryHealthStatus {
    ...
    async fn initialize_local_registry_info(config: &RegistryConfig) -> BearDogResult<LocalRegistryInfo> {
        // Generate or load cryptographic keys
        let (public_key, private_key) = crate::crypto_utils::BearDogCrypto::generate_ed25519_keypair()?;
...
impl NodeRegistry for BearDogNodeRegistry {
    fn get_node_info(&self, node_id: &str) -> BearDogResult<crate::auth::types::NodeInfo> {
        ...
        Err(BearDogError::config("Async trait support needed - use async methods instead"))
    }
