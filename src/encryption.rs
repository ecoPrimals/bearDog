...
impl EncryptionEngine {
    ...
    async fn decrypt_aes256_gcm(&self, encrypted_data: &EncryptedData) -> BearDogResult<Vec<u8>> {
        ...
        let key_bytes = encrypted_data.metadata.get("key").ok_or_else(|| {
            BearDogError::encryption("aes_decryption", "Key not found in metadata")
        })?;
    ...
    async fn decrypt_chacha20_poly1305(
        ...
        // Retrieve the key from metadata
        let key_bytes = encrypted_data.metadata.get("key").ok_or_else(|| {
            BearDogError::encryption("chacha20_decryption", "Key not found in metadata")
        })?;
    ...
    async fn decrypt_quantum_resistant(
            ...
            || encrypted_data.metadata.get("layers") != Some(&"2".to_string())
        {
            return Err(BearDogError::encryption(
                "quantum_decryption",
                "Invalid quantum-resistant encrypted data format",
            ));
        }
        ...
        if encrypted_data.nonce.len() != 24 {
            ...
            return Err(BearDogError::encryption(
                "quantum_decryption",
                "Invalid nonce length for quantum-resistant decryption",
            ));
        }
    ...
    pub async fn generate_master_key(&self) -> BearDogResult<String> {
        self.generate_key("AES256", "master")
            .await
            .map(|(id, _)| id)
    }
