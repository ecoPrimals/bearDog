...
impl ThreatDetectionEngine {
    ...
    async fn add_asset_context(&self, threat_event: &mut ThreatEvent) -> BearDogResult<()> {
        ...
        // Add asset evidence
        let hostname_display = threat_event
            .target
            .hostname.as_deref()
            .unwrap_or("Unknown");
    ...
    async fn simulate_network_context_lookup(
        ...
        if ip_address.starts_with("192.168") {
            Ok(Some(NetworkContextInfo {
                description: "Internal corporate network".to_string(),
            ...
            Ok(Some(NetworkContextInfo {
                description: "External internet connection".to_string(),
...
pub struct NetworkContextInfo {
    /// Description of the network context
    pub description: String,
