...
impl SystematicUnwrapMigrator {
    ...
    beardog_errors::BearDogError::ConfigurationError({}", "$1"))
})?"#.to_string(BearDogErrorCategory::Configuration,
    ...
    beardog_errors::BearDogError::ConfigurationError({}", "$1", "$2"))
})?"#.to_string(BearDogErrorCategory::Configuration,
        ...
        error_patterns.insert(
            ...
            MigrationPattern {
                ...
                replacement: ".lock().unwrap_or_else(|poisoned| {\n        tracing::warn!(\"Mutex poisoned, recovering\");\n        poisoned.into_inner(BearDogErrorCategory::System,
    ...
    beardog_errors::BearDogError::ValidationError({}", e))
})?"#.to_string(BearDogErrorCategory::Validation,
    ...
    beardog_errors::BearDogError::ValidationError({}", "$2", e))
})?"#.to_string(BearDogErrorCategory::Validation,
    ...
    beardog_errors::BearDogError::NetworkError({}", e))
})?"#.to_string(BearDogErrorCategory::Network,
    ...
    beardog_errors::BearDogError::NetworkError({}", "$1", e))
})?"#.to_string(BearDogErrorCategory::Network,
    ...
    beardog_errors::BearDogError::StorageError({}", e))
})?"#.to_string(BearDogErrorCategory::Storage,
    ...
    beardog_errors::BearDogError::SecurityError({}", e))
})?"#.to_string(BearDogErrorCategory::Security,
    ...
    tracing::error!("Environment variable '{}' not found: {}", "$1", e);
    beardog_errors::BearDogError::config({}", "$1"))
})?"#.to_string(BearDogErrorCategory::Configuration,
                context: "Environment variable access".to_string(true,
            }
        );
    ...
    beardog_errors::BearDogError::validation({:?}", e))
})?"#.to_string(BearDogErrorCategory::Validation,
    ...
    beardog_errors::BearDogError::validation({:?}", "$1", e))
})?"#.to_string(BearDogErrorCategory::Validation,
    ...
    beardog_errors::BearDogError::validation(BearDogErrorCategory::Validation,
    ...
    beardog_errors::BearDogError::validation(BearDogErrorCategory::Validation,
        ...
        error_patterns.insert(
            "result_unwrap_beardog".to_string(),
            MigrationPattern {
                pattern: r#"\.map\([^)]+\)\.unwrap\(\)"#.to_string(r#".map_err(|e| {
    tracing::error!("Operation failed: {:?}", e);
    beardog_errors::BearDogError::internal({:?}", e))
})?
.map(BearDogErrorCategory::System,
                context: "Result operation with map".to_string(true,
            }
        );

        error_patterns.insert(
            "option_unwrap_beardog".to_string(),
            MigrationPattern {
                pattern: r#"\.as_ref\(\)\.unwrap\(\)"#.to_string(),
                replacement: r#".as_ref().ok_or_else(|| {
    tracing::error!("Operation failed: None value");
    beardog_errors::BearDogError::internal(BearDogErrorCategory::System,
                context: "Option as_ref operation".to_string(true,
            }
        );

        error_patterns.insert(
            "option_expect_beardog".to_string(),
            MigrationPattern {
                pattern: r#"\.as_ref\(\)\.expect\("([^"]+)"\)"#.to_string(),
                replacement: r#".as_ref().ok_or_else(|| {
    tracing::error!("Operation failed ({})", "$1");
    beardog_errors::BearDogError::internal(BearDogErrorCategory::System,
        ...
        error_patterns.insert(
            ...
            MigrationPattern {
                ...
                replacement: r#".map_err(|e| BearDogError::system(format!("Operation failed: {:?}", e)))?"#.to_string(),
        ...
        error_patterns.insert(
            ...
            MigrationPattern {
                ...
                replacement: r#".map_err(|e| BearDogError::system(format!("$1 failed: {:?}", e)))?"#.to_string(),
    ...
    beardog_errors::BearDogError::internal({:?}", "Failed to create async runtime for example", e))
})?"#.to_string(BearDogErrorCategory::System,
    ...
    beardog_errors::BearDogError::internal({:?}", "JSON serialization failed in example", e))
})?"#.to_string(BearDogErrorCategory::System,
    ...
    beardog_errors::BearDogError::ValidationError({}", "JSON deserialization failed in example", e))
})?"#.to_string(BearDogErrorCategory::System,
            context: "Example JSON deserialization".to_string(false,
        });
        
        error_patterns.insert("example_sort_unwrap".to_string(), MigrationPattern {
            pattern: r"\.sort_by\([^)]*\.partial_cmp\([^)]+\)\.unwrap\(\)[^)]*\)".to_string(r".sort_by(|a, b| a.partial_cmp(b).unwrap_or(std::cmp::Ordering::Equal))".to_string(BearDogErrorCategory::System,
    ...
    beardog_errors::BearDogError::internal({:?}", "Benchmark runtime creation failed", e))
})?"#.to_string(BearDogErrorCategory::System,
    ...
    beardog_errors::BearDogError::internal({:?}", "Benchmark buffer creation failed", e))
})?"#.to_string(BearDogErrorCategory::System,
    ...
    beardog_errors::BearDogError::internal({:?}", "Benchmark key generation failed", e))
})?"#.to_string(BearDogErrorCategory::Security,
        ...
        error_patterns.insert("benchmark_result_unwrap".to_string(), MigrationPattern {
            ...
            replacement: r"results.into_iter(BearDogErrorCategory::System,
        ...
        error_patterns.insert("production_panic_auth".to_string(), MigrationPattern {
            ...
            replacement: r#"return Err(BearDogError::internal(BearDogErrorCategory::System,
        ...
        error_patterns.insert("production_panic_serialization".to_string(), MigrationPattern {
            ...
            replacement: r#"return Err(BearDogError::serialization(format!("Serialization failed: {e:?}")))"#.to_string(BearDogErrorCategory::System,
            context: "Serialization error handling".to_string(true,
        });

        error_patterns.insert("test_panic_setup".to_string(), MigrationPattern {
            pattern: r#"panic!\("Test failed to (\w+): \{e:\?\}"\)"#.to_string(r#"panic!("Test setup failed during {}: {:?}", "$1", e)"#.to_string(BearDogErrorCategory::System,
            context: "Test setup failure".to_string(false,
        });
        
        error_patterns.insert("test_panic_assertion".to_string(), MigrationPattern {
            pattern: r#"panic!\("Expected (\w+) error, got \{error:\?\}"\)"#.to_string(r#"panic!("Expected {} error, got {:?}", "$1", error)"#.to_string(BearDogErrorCategory::System,
        ...
        if !dry_run && migrations_applied > 0 {
            fs::write(&str, position: usize) -> bool {
