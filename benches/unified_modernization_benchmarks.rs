...
fn benchmark_unified_config(c: &mut Criterion) {
    ...
    // Benchmark security config with environment loading
    group.bench_function("security_config_from_env", |b| {
    ...
    // Benchmark app config validation
    group.bench_function("app_config_validation", |b| {
...
fn benchmark_modular_vs_monolithic(c: &mut Criterion) {
    ...
    group.bench_function("modular_config_access", |b| {
        ...
        b.iter(|| {
            ...
            let _ = black_box(&security_config.authentication);
        })
    });
...
fn benchmark_config_validation_scaling(c: &mut Criterion) {
    ...
    for size in [1, 10, 100].iter() {
        group.bench_with_input(
            BenchmarkId::new("provider_config_validation", size),
            size,
            |b, &size| {
...
fn benchmark_modernization_benefits(c: &mut Criterion) {
    ...
    group.bench_function("unified_configuration_performance", |b| {
        b.iter(|| {
            ...
            // Simulate unified validation
            let provider_valid = provider_config.validate().is_ok();
            let security_valid = security_config.validate().is_ok();
