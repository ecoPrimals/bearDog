...
async fn test_ecosystem_membership_spectrum() {
    ...
    let steward = EcosystemMembership::CoreSteward {
        stewardship_areas: vec!["security".to_string(), "architecture".to_string()],
        trust_level: 0.95,
    };
    ...
    let contributor = EcosystemMembership::ActiveContributor {
        contribution_types: vec!["code".to_string(), "documentation".to_string()],
    ...
    let learner = EcosystemMembership::LearningParticipant {
        ...
        progress_metrics: ProgressMetrics {
            skills_acquired: vec!["rust_basics".to_string()],
...
async fn test_trust_evolution_states() {
    // Test Building trust
    let building_trust = TrustEvolution::Building {
        progress_rate: 0.5,
    ...
    let flourishing_trust = TrustEvolution::Flourishing {
        ...
        mutual_benefit_indicators: vec![BenefitMetric {
            metric_name: "code_quality".to_string(),
            value: 0.8,
    ...
    // Test Healing trust
    let healing_trust = TrustEvolution::Healing {
        ...
        trust_repair_actions: vec![RepairAction {
            action_type: "community_service".to_string(),
...
async fn test_coordination_models() {
    ...
    let rotational = CoordinationModel::Rotational(RotationCriteria {
    ...
    let collaborative = CoordinationModel::Collaborative(DecisionProtocol {
        decision_method: "consensus".to_string(),
...
async fn test_ecosystem_protection_with_restoration() {
    let protection = EcosystemMembership::EcosystemProtection {
        protection_level: ProtectionLevel::LimitedInteraction,
...
async fn test_concern_factors_and_monitoring() {
    let cautious_interaction = EcosystemMembership::CautiousInteraction {
        concern_factors: vec![
...
async fn test_visiting_collaborator() {
    let visitor = EcosystemMembership::VisitingCollaborator {
