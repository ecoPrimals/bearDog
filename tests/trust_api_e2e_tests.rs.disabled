...
async fn test_trust_identity_with_family() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(
        encryption_tag.contains("test"),
        "encryption_tag should contain 'test', got: {}",
        encryption_tag
    );
...
async fn test_trust_identity_without_family() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(
        encryption_tag.starts_with("beardog:node:")
            || encryption_tag.starts_with("beardog:family:"),
        "encryption_tag should start with beardog:node: or beardog:family:, got: {}",
        encryption_tag
    );

    // Should NOT have family_id
    assert!(
        body["data"]["family_id"].is_null(),
        "family_id should be null when no seed provided"
    );
...
async fn test_trust_health_includes_trust_capability() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(
        has_trust,
        "Health endpoint should include 'trust' capability"
    );
