...
async fn test_hsm_error_handling() -> Result<(), BearDogError> {
    ...
    test_invalid_key_operations(&mut harness)?;
    test_provider_unavailability(&mut harness)?;
    test_resource_exhaustion(&mut harness)?;
...
async fn test_invalid_key_operations(harness: &mut HsmTestHarness) -> Result<(), BearDogError> {
    ...
    assert!(
        result.is_err(),
        "Operation with non-existent key should fail"
    );

    let result = harness.software_hsm.generate_invalid_key();
...
async fn test_provider_unavailability(harness: &mut HsmTestHarness) -> Result<(), BearDogError> {
    ...
    let result = harness.hsm_manager.handle_provider_failure()?;
    assert!(
...
async fn test_resource_exhaustion(harness: &mut HsmTestHarness) -> Result<(), BearDogError> {
    ...
    let result = harness.hsm_manager.test_resource_limits()?;
    assert!(
        result.handles_exhaustion,
...
async fn test_hsm_system_integration_e2e() -> Result<(), BearDogError> {
    ...
    test_complete_workflow(&mut harness)?;
    test_cross_provider_operations(&mut harness)?;
    test_system_recovery(&mut harness)?;
...
async fn test_complete_workflow(harness: &mut HsmTestHarness) -> Result<(), BearDogError> {
    ...
    let system_status = harness.hsm_manager.get_system_status()?;
    assert!(system_status.operational, "System should be operational");

    let strongbox_key = harness
        .android_strongbox
        .generate_test_key("e2e_strongbox")
        ?;
    let software_key = harness
        .software_hsm
    ...
    let strongbox_sig = harness
        ...
        ?;
    let software_sig = harness
        .software_hsm
        .sign(&software_key.key_id, test_data)
        ?;

    let strongbox_valid = harness
        .android_strongbox
        .verify(&strongbox_key.key_id, test_data, &strongbox_sig)
        ?;
    let software_valid = harness
        .software_hsm
        .verify(&software_key.key_id, test_data, &software_sig)
        ?;

    assert!(
        strongbox_valid && software_valid,
...
async fn test_cross_provider_operations(harness: &mut HsmTestHarness) -> Result<(), BearDogError> {
    ...
    let result = harness
        .hsm_manager
        .execute_cross_provider_operation()
        ?;
...
async fn test_system_recovery(harness: &mut HsmTestHarness) -> Result<(), BearDogError> {
    ...
    let recovery_result = harness.hsm_manager.test_system_recovery()?;
    assert!(
...
pub async fn run_comprehensive_hsm_tests() -> Result<(), BearDogError> {
    ...
    test_hsm_error_handling()?;
    test_hsm_system_integration_e2e()?;
