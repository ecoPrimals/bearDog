...
async fn test_e2e_http_response_body_warnings() {
    ...
    assert_eq!(response_body["protocol_warning"], "HTTP is less secure than JSON-RPC");
...
async fn test_e2e_protocol_detection_performance() {
    ...
    assert!(elapsed.as_micros() < 1000, "Protocol detection too slow: {:?}", elapsed);
}
