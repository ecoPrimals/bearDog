...
async fn test_basic_encryption_decryption() -> BearDogResult<()> {
    ...
    // Test encryption
    let encrypted = engine.encrypt(test_data, None).await?;
    assert!(encrypted.ciphertext.len() > 0, "Encrypted data should not be empty");
...
async fn test_key_derivation() -> BearDogResult<()> {
    ...
    let derived_key = crypto_utils::pbkdf2_hmac_sha256(password.as_bytes(), salt.as_bytes(), 10000, 32);
    assert_eq!(derived_key.len(), 32);
    
    // Test consistency - same input should produce same output
    let derived_key2 = crypto_utils::pbkdf2_hmac_sha256(password.as_bytes(), salt.as_bytes(), 10000, 32);
    assert_eq!(derived_key, derived_key2);
    
    // Test different passwords produce different keys
    let different_key = crypto_utils::pbkdf2_hmac_sha256("different_password".as_bytes(), salt.as_bytes(), 10000, 32);
...
async fn test_encryption_with_large_data() -> BearDogResult<()> {
    ...
    println!("✅ Large data encryption/decryption successful: {} bytes", large_data.len());
