...
async fn test_zero_cost_cache_performance_regression() {
    ...
    let result = PerformanceTestResult {
        ...
        failure_reason: if ops_per_second < PERFORMANCE_THRESHOLDS.cache_ops_per_second as f64 {
            Some(format_args!("Cache performance below threshold: {:.0} < {}", 
                        ops_per_second, PERFORMANCE_THRESHOLDS.cache_ops_per_second).to_string())
        } else if allocations > PERFORMANCE_THRESHOLDS.max_heap_allocations {
            Some(format_args!("Excessive memory allocations: {} > {}", 
                        allocations, PERFORMANCE_THRESHOLDS.max_heap_allocations).to_string())
        } else {
            None
        },
    };
...
async fn test_zero_cost_security_performance_regression() {
    ...
    for i in 0..ITERATIONS {
        let credentials = HashMap::from([
            ("username".to_string(), format_args!("perf_user_{}", i).to_string()),
    ...
    let result = PerformanceTestResult {
        ...
        failure_reason: if ops_per_second < PERFORMANCE_THRESHOLDS.security_ops_per_second as f64 {
            Some(format_args!("Security performance below threshold: {:.0} < {}", 
                        ops_per_second, PERFORMANCE_THRESHOLDS.security_ops_per_second).to_string())
        } else if avg_latency_ms > 0.1 {
            Some(format_args!("Security latency too high: {:.3}ms > 0.1ms", avg_latency_ms).to_string())
        } else {
            None
        },
    };
...
async fn test_zero_cost_workflow_performance_regression() {
    ...
    let result = PerformanceTestResult {
        ...
        failure_reason: if ops_per_second < PERFORMANCE_THRESHOLDS.workflow_ops_per_second as f64 {
            Some(format_args!("Workflow performance below threshold: {:.0} < {}", 
                        ops_per_second, PERFORMANCE_THRESHOLDS.workflow_ops_per_second).to_string())
        } else if avg_latency_ms > PERFORMANCE_THRESHOLDS.workflow_latency_ms {
            Some(format_args!("Workflow latency too high: {:.3}ms > {:.1}ms", 
                        avg_latency_ms, PERFORMANCE_THRESHOLDS.workflow_latency_ms).to_string())
        } else {
            None
        },
    };
...
async fn test_full_stack_integration_performance_regression() {
    ...
    for i in 0..ITERATIONS {
        ...
        let credentials = HashMap::from([
            ("username".to_string(), format_args!("integration_user_{}", i).to_string()),
    ...
    format_args!("Operation failed: {:?}", e).to_string()
        ...
        let cache_key = format_args!("integration_key_{}", i).to_string();
        let cache_data = format_args!("integration_data_{}", i).to_string().into_bytes();
    ...
    let result = PerformanceTestResult {
        ...
        failure_reason: if ops_per_second < PERFORMANCE_THRESHOLDS.full_stack_ops_per_second as f64 {
            Some(format_args!("Integration performance below threshold: {:.0} < {}", 
                        ops_per_second, PERFORMANCE_THRESHOLDS.full_stack_ops_per_second).to_string())
        } else if avg_latency_ms > PERFORMANCE_THRESHOLDS.full_stack_latency_ms {
            Some(format_args!("Integration latency too high: {:.3}ms > {:.1}ms", 
                        avg_latency_ms, PERFORMANCE_THRESHOLDS.full_stack_latency_ms).to_string())
        } else {
            None
        },
    };
...
async fn test_memory_allocation_regression() {
    ...
    for i in 0..100 {
        let key = format_args!("memory_test_{}", i).to_string();
        let value = format_args!("value_{}", i).to_string();
...
async fn test_concurrency_performance_regression() {
    ...
    for task_id in 0..CONCURRENT_TASKS {
        ...
        let task = tokio::spawn(async move {
            for op_id in 0..OPERATIONS_PER_TASK {
                ...
                let credentials = HashMap::from([
                    ("username".to_string(), format_args!("concurrent_user_{}_{}", task_id, op_id).to_string()),
    ...
    format_args!("Operation failed: {:?}", e).to_string()
                ...
                let cache_key = format_args!("concurrent_key_{}_{}", task_id, op_id).to_string();
                let cache_value = format_args!("concurrent_value_{}_{}", task_id, op_id).to_string().into_bytes();
...
fn create_performance_test_workflow(id: usize) -> beardog_workflows::workflows::types::Workflow {
    ...
    Workflow {
        ...
        target: WorkflowTarget::Key { id: format_args!("perf_key_{}", id).to_string() },
        ...
        requested_by: format_args!("perf_user_{}", id).to_string(),
        initiator: format_args!("perf_user_{}", id).to_string(),
        description: format_args!("Performance test workflow {}", id).to_string(),
        metadata: HashMap::with_capacity(16),
        timeout_duration: None,
        properties: HashMap::with_capacity(16),
        parameters: HashMap::from([
            ("key_id".to_string(), serde_json::Value::String(format_args!("perf_key_{}", id).to_string())),
