...
async fn test_authorization_throughput() {
    ...
    for i in 0..num_requests {
        ...
        let (response, _) = send_jsonrpc_request_timed("graph.authorize_modification", params, socket_path).await;
    ...
    println!("Average latency: {:.2} ms", elapsed.as_millis() as f64 / num_requests as f64);
    
    // Verify we can handle at least 50 req/sec
    assert!(throughput > 50.0, "Throughput should be > 50 req/sec, got {:.2}", throughput);
...
async fn test_validation_throughput() {
    ...
    for i in 0..num_requests {
        ...
        let (response, _) = send_jsonrpc_request_timed("graph.validate_template", params, socket_path).await;
        assert_eq!(response["jsonrpc"], "2.0");
    }
...
async fn test_authorization_latency_p95() {
    ...
    for i in 0..100 {
        ...
        let (response, latency) = send_jsonrpc_request_timed("graph.authorize_modification", params, socket_path).await;
        assert_eq!(response["jsonrpc"], "2.0");
...
async fn test_concurrent_authorization_requests() {
    ...
    println!("Concurrent throughput: {:.2} req/sec ({} clients, {} req each)", 
...
async fn test_sustained_load() {
    ...
    println!("Sustained load throughput: {:.2} req/sec over {} seconds", 
             throughput, elapsed.as_secs());
    
    // Verify sustained performance
    assert!(throughput > 30.0, "Sustained throughput should be > 30 req/sec");
