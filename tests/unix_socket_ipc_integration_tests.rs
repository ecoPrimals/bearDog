...
async fn create_test_btsp_provider() -> Arc<BeardogBtspProvider> {
    ...
    let hsm = Arc::new(HsmManager::auto_initialize().await.expect("HSM init failed"));
    let genetics = Arc::new(EcosystemGeneticEngine::new().expect("Genetics init failed"));
    
    Arc::new(BeardogBtspProvider::new(hsm, genetics).await.expect("BTSP provider init failed"))
}
...
async fn start_server_ready(
    ...
    assert!(
        UnixSocketIpcServer::wait_ready_flag(&ready_flag, tokio::time::Duration::from_secs(10)).await,
...
async fn test_wait_ready_timeout() {
    ...
    let result = server.wait_ready(std::time::Duration::from_millis(100)).await;
