...
async fn fault_test_socket_deletion_during_operation() {
    ...
    assert!(result.is_ok(), "Existing connection should work after socket deletion");
...
async fn fault_test_malformed_requests() {
    ...
    assert!(result.is_ok(), "Should handle malformed requests gracefully");
...
async fn fault_test_partial_writes() {
    ...
    use tokio::io::AsyncWriteExt;
    stream.write_all(b"{\"jsonrpc\":\"2.0\",\"method\":\"health\"").await.unwrap();
    stream.flush().await.unwrap();
