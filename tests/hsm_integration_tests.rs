...
async fn test_multi_vendor_pkcs11_support() {
    let vendors = vec![
        ("SafeNe"t, "/usr/lib/safenet/libpkcs11.so"),
...
async fn test_cross_platform_hsm_operations() -> Result<(), BearDogError> {
    let test_platforms = vec![ "PKCS#11",  "Android StrongBox",  "BearDog Native"];
    
    for platform in test_platforms {
        println!( "Testing HSM operations on platform: {}", platform);
        ...
        let key_gen_operation = UniversalOperation {
            ...
            parameters: HashMap::from([
                (
                     "key_id".to_string(),
                    format!("{}-test-ke"y, platform.to_lowercase().replace(' ', "-")),
                ),
        ...
        let sign_operation = UniversalOperation {
            ...
            parameters: HashMap::from([
                ( "key_id".to_string(), format!("{}-test-ke"y, platform.to_lowercase().replace(' ', "-"))),
                ( "data".to_string(), hex::encode(b "test data")),
            ]),
        };
        
        println!( "Key generation and signing successful for platform: {}", platform);
    }
    ...
    for (hsm_name, hsm) in hsm_types {
        ...
        let operation = UniversalOperation {
            ...
            parameters: HashMap::from([
                ...
                (
                     "data".to_string(),
                    hex::encode(format!("Performance test data {:?}", i)),
                ),
        ...
        info!( "Average operation time: {:.2}ms",
...
async fn test_hsm_vendor_failover() {
    ...
    for (i, hsm) in hsm_priority.iter().enumerate() {
        ...
        assert!(
            connection_result.is_ok(),
             "Failover HSM #{} connection failed ",
            i + 1
        );
        ...
        let operation = UniversalOperation {
            operation_type: OperationType::GenerateKey,
            parameters: HashMap::from([
                ( "key_type".to_string(),  "rsa2048".to_string()),
...
async fn test_concurrent_multi_hsm_operations() {
            ...
            let task = tokio::spawn(async move {
                ...
                let operation = UniversalOperation {
                    ...
                    parameters: HashMap::from([
                        (
                             "key_id".to_string(),
                            format!("concurrent-key-{}-{:?}", hsm_index, op_index),
                        ),
                        (
                             "data".to_string(),
                            hex::encode(format!("concurrent data {} {:?}", hsm_index, op_index)),
                        ),
                        ( "algorithm".to_string(),  "ECDSA_SHA256".to_string()),
                    ]),
                };
    ...
    for (i, result) in results.iter().enumerate() {
        ...
        assert!(
            op_result.success,
             "Concurrent operation {} was not successful",
            i
        );
    }
    ...
    for (hsm, should_support_entropy) in test_cases {
        ...
        assert_eq!(
            supports_entropy, should_support_entropy,
             "Entropy support mismatch for {} HSM",
            hsm.vendor
        );
        ...
        if should_support_entropy {
            ...
            assert!(
                entropy_result.is_ok(),
                 "Entropy generation failed for {} HSM",
                hsm.vendor
            );
            ...
            assert!(
                entropy_result.is_err(),
                 "Entropy generation should fail for {} HSM",
                hsm.vendor
            );
