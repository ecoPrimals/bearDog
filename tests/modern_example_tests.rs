...
mod common;
use common::{
    assert_duration_within, assert_error_contains, assert_in_range, assert_success,
    create_test_data, global_fixtures, BearDogTestHarness, ErrorMatcher, HttpResponseMatcher,
    PerformanceMatcher, TestContext, TestEnvironment, TestHarnessConfig, TestMatcher, TestResult,
};
...
async fn modern_unit_test_example() -> TestResult<()> {
    ...
    let mut harness = BearDogTestHarness::with_config(TestEnvironment::Unit,
    ...
    let result = harness
        .run_test("config_parsing_validation", |context, core| async move {
            let fixtures = global_fixtures();
            let config_data = fixtures
                .get_dataset("test_config")
            ...
            let config_str = serde_json::to_string_pretty(config_data).map_err(|e| {
                beardog_errors::BearDogError::serialization({}",
                    e
                ))
            })?;
...
async fn modern_integration_test_example() -> TestResult<()> {
    ...
    let mut harness = BearDogTestHarness::with_config(TestEnvironment::Integration,
        enable_performance_monitoring: true,
    ...
    let result = harness
        .run_test(
            "crypto_security_integration",
            |mut context, core| async move {
                context.start_phase(common::TestPhase::Setup);

                let fixtures = global_fixtures();
                let crypto_fixtures = fixtures.crypto();
                let sample_keypair = crypto_fixtures.get_ed25519_keypair(0).ok_or_else(|| {
                    beardog_errors::BearDogError::not_found("Sample Ed25519 keypair")
                })?;
                ...
                let crypto_matcher =
                    TestMatcher::new("crypto_security_validation")
                        ...
                        .add_matcher(|ctx, _value| common::MatchResult {
                            ...
                            suggestions: vec![
                                "Continue with current crypto implementation".to_string()
                            ],
                        });
                ...
                if !is_valid {
                    return Err(beardog_errors::BearDogError::authentication(
...
async fn modern_e2e_test_example() -> TestResult<()> {
    ...
    let mut harness = BearDogTestHarness::with_config(TestEnvironment::E2E,
        enable_performance_monitoring: true,
        enable_auto_cleanup: true,
    ...
    let result = harness
        .run_test("full_system_workflow", |mut context, core| async move {
            ...
            let e2e_matcher = TestMatcher::new("full_system_validation")
                ...
                .add_matcher(|ctx, _value| {
                    ...
                    common::MatchResult {
                        ...
                        details: [
                            (
                                "workflow_duration".to_string(),
                                serde_json::json!(ctx.start_time.elapsed().as_secs()),
                            ),
                            (
                                "error_count".to_string(),
                                serde_json::json!(ctx.performance_metrics.errors_encountered),
                            ),
                        ]
                        .into(),
            ...
            let validation_result = e2e_matcher.validate(
                &context,
                &serde_json::json!({
                    "workflow ": "completed ",
...
async fn modern_error_handling_test_example() -> TestResult<()> {
    ...
    let result = harness
        .run_test("error_handling_validation", |context, core| async move {
            ...
            let timeout_error = beardog_errors::BearDogError::timeout(Result<String, beardog_errors::BearDogError> = Err(timeout_error);
            assert_error_contains(&timeout_result, "timed out", None)?;

            let config_error = beardog_errors::BearDogError::configuration(
...
async fn modern_performance_benchmark_example() -> TestResult<()> {
    ...
    let mut harness = BearDogTestHarness::with_config(TestEnvironment::Performance,
        enable_performance_monitoring: true,
        enable_auto_cleanup: true,
        default_timeout: Duration::from_secs(30),
        ..Default::default()
    ...
    let result = harness
        .run_test(
            ...
            |mut context, core| async move {
                ...
                for i in 0..100 {
                    let keypair =
                        beardog_security::crypto_utils::BearDogCrypto::generate_ed25519_keypair()
                            .map_err(|e| {
                            beardog_errors::BearDogError::crypto({}",
                                i, e
                            ))
                        })?;
                    ...
                    if !is_valid {
                        return Err(beardog_errors::BearDogError::authentication(&format!(
                            "Signature verification failed at iteration {}",
                            i
                        )));
                    }
                }
...
async fn modern_test_suite_example() -> TestResult<()> {
    ...
    suite_runner
        .run_test("basic_initialization", |context, core| async move {
    ...
    suite_runner
        .run_test("configuration_loading", |context, core| async move {
            info!("Testing configuration loading");
            let fixtures = global_fixtures();
            let config = fixtures.get_config("unit").ok_or_else(|| {
                beardog_errors::BearDogError::not_found("Unit test configuration")
            })?;
    ...
    suite_runner
        .run_test("security_subsystem", |context, core| async move {
    ...
    info!(
        ...
        report.metrics.average_test_duration
    );
    ...
    Ok(&str, delay_ms: u64) -> TestResult<serde_json::Value> {
