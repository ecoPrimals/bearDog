...
async fn test_universal_ecosystem_manager() -> BearDogResult<()> {
    ...
    let beardog_provider =
        beardog_provider.map_err(|e| BearDogError::system({:?}", e)))?;
...
mod test_helpers {
    ...
    impl PrimalProvider for MockComputeServiceProvider {
        ...
        fn capabilities(&self) -> Vec<Capability> {
            ...
            type_field: CapabilityCategory::Compute,
...
async fn test_multi_provider_ecosystem() -> BearDogResult<()> {
    ...
    let toadstool_provider =
        test_helpers::MockComputeServiceProvider::new("toadstool-001".to_string());
    manager
        .register_provider(
            toadstool_provider,
            ProviderConfig {
